    ...
    if(USE_NVIDIA_NPP)
        ...
        foreach(npp_lib ${NPP_LIBS})
            ...
            if(${npp_lib}_LIBRARY)
                ...
                string(FIND "${${npp_lib}_LIBRARY}" "${CUDA_TOOLKIT_ROOT_DIR}" pos)
                if(pos EQUAL -1)
                    message(FATAL_ERROR "Found NPP library ${${npp_lib}_LIBRARY} is NOT from detected CUDA ${CUDA_TOOLKIT_ROOT_DIR}! This will cause linking errors.")
                endif()
                
                list(APPEND NPP_FOUND_LIBS ${${npp_lib}_LIBRARY})
                message(STATUS "Found NPP library: ${${npp_lib}_LIBRARY}")
            else()
                message(FATAL_ERROR "NPP library not found: ${npp_lib} in ${NPP_SEARCH_PATHS}")
            endif()
        endforeach()
        
        # 一次性链接所有找到的库，保证顺序
        if(NPP_FOUND_LIBS)
            target_link_libraries(${target_name} PRIVATE ${NPP_FOUND_LIBS})
        endif()
