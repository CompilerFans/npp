...
TEST_F(DivFunctionalTest, Div_8u_C1RSfs_BasicOperation) {
  ...
  for (size_t i = 0; i < resultData.size(); i++) {
    NPP_EXPECT_ARITHMETIC_EQUAL(resultData[i], expectedData[i], "nppiDiv_8u_C1RSfs")
        << "Div operation mismatch at index " << i << ": got " << (int)resultData[i] << ", expected "
        << (int)expectedData[i];
  }
}
...
TEST_F(DivFunctionalTest, Div_32f_C1R_BasicOperation) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiDiv_32f_C1R(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi);
  ...
  for (size_t i = 0; i < resultData.size(); i++) {
    NPP_EXPECT_ARITHMETIC_EQUAL(resultData[i], expectedData[i], "nppiDiv_32f_C1R")
        << "Div 32f operation mismatch at index " << i << ": got " << resultData[i] << ", expected " << expectedData[i];
  }
}
