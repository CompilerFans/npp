...
class XorC8uC3ParamTest : public NppTestBase, public ::testing::WithParamInterface<XorC8uParam> {};

TEST_P(XorC8uC3ParamTest, XorC_8u_C3R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  const Npp8u constant = param.constant;
  ...
  std::vector<Npp8u> expectedData(total);
  for (size_t i = 0; i < expectedData.size(); i++) {
    expectedData[i] = expect::xor_c<Npp8u>(srcData[i], aConstants[i % 3]);
  }
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXorC_8u_C3IR_Ctx(aConstants, src.get(), src.step(), roi, ctx);
    } else {
      status = nppiXorC_8u_C3IR(aConstants, src.get(), src.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXorC_8u_C3R_Ctx(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiXorC_8u_C3R(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(XorC8uC3, XorC8uC3ParamTest,
                         ::testing::Values(XorC8uParam{32, 32, 0xAA, false, false, "32x32_cAA_noCtx"},
                                           XorC8uParam{32, 32, 0xAA, true, false, "32x32_cAA_Ctx"},
                                           XorC8uParam{32, 32, 0x55, false, true, "32x32_c55_InPlace"},
                                           XorC8uParam{32, 32, 0x55, true, true, "32x32_c55_InPlace_Ctx"}),
...
class XorC8uC4ParamTest : public NppTestBase, public ::testing::WithParamInterface<XorC8uParam> {};

TEST_P(XorC8uC4ParamTest, XorC_8u_C4R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  const Npp8u constant = param.constant;
  ...
  std::vector<Npp8u> expectedData(total);
  for (size_t i = 0; i < expectedData.size(); i++) {
    expectedData[i] = expect::xor_c<Npp8u>(srcData[i], aConstants[i % 4]);
  }
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXorC_8u_C4IR_Ctx(aConstants, src.get(), src.step(), roi, ctx);
    } else {
      status = nppiXorC_8u_C4IR(aConstants, src.get(), src.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXorC_8u_C4R_Ctx(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiXorC_8u_C4R(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(XorC8uC4, XorC8uC4ParamTest,
                         ::testing::Values(XorC8uParam{32, 32, 0xAA, false, false, "32x32_cAA_noCtx"},
                                           XorC8uParam{32, 32, 0xAA, true, false, "32x32_cAA_Ctx"},
                                           XorC8uParam{32, 32, 0x55, false, true, "32x32_c55_InPlace"},
                                           XorC8uParam{32, 32, 0x55, true, true, "32x32_c55_InPlace_Ctx"}),
...
class Xor16uC1ParamTest : public NppTestBase, public ::testing::WithParamInterface<Xor16uParam> {};

TEST_P(Xor16uC1ParamTest, Xor_16u_C1R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXor_16u_C1IR_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), roi, ctx);
    } else {
      status = nppiXor_16u_C1IR(src1.get(), src1.step(), src2.get(), src2.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXor_16u_C1R_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiXor_16u_C1R(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(Xor16uC1, Xor16uC1ParamTest,
                         ::testing::Values(Xor16uParam{32, 32, false, false, "32x32_noCtx"},
                                           Xor16uParam{32, 32, true, false, "32x32_Ctx"},
                                           Xor16uParam{32, 32, false, true, "32x32_InPlace"},
                                           Xor16uParam{32, 32, true, true, "32x32_InPlace_Ctx"}),
                         [](const ::testing::TestParamInfo<Xor16uParam> &info) { return info.param.name; });

// ==================== Xor 16u C3 TEST_P ====================

class Xor16uC3ParamTest : public NppTestBase, public ::testing::WithParamInterface<Xor16uParam> {};

TEST_P(Xor16uC3ParamTest, Xor_16u_C3R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  const int channels = 3;
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXor_16u_C3IR_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), roi, ctx);
    } else {
      status = nppiXor_16u_C3IR(src1.get(), src1.step(), src2.get(), src2.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiXor_16u_C3R_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiXor_16u_C3R(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(Xor16uC3, Xor16uC3ParamTest,
                         ::testing::Values(Xor16uParam{32, 32, false, false, "32x32_noCtx"},
                                           Xor16uParam{32, 32, true, false, "32x32_Ctx"},
                                           Xor16uParam{32, 32, false, true, "32x32_InPlace"},
                                           Xor16uParam{32, 32, true, true, "32x32_InPlace_Ctx"}),
