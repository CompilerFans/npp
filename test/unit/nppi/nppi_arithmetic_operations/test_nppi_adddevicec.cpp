...
TEST_F(NppiAddDeviceCTest, AddDeviceC_8u_C1RSfs_WithScaling) {
    ...
    NppStreamContext nppStreamCtx;
    nppGetStreamContext(&nppStreamCtx);
    NppStatus status = nppiAddDeviceC_8u_C1RSfs_Ctx(d_src, step, d_constant, d_dst, step, oSizeROI, scaleFactor, nppStreamCtx);
    ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiAddDeviceCTest, AddDeviceC_8u_C3RSfs_MultiChannel) {
    ...
    std::vector<Npp8u> expected = {
        // Pixel (0,0): R=10+5=15, G=20+10=30, B=30+15=45
        15, 30, 45,
    ...
    NppStreamContext nppStreamCtx;
    nppGetStreamContext(&nppStreamCtx);
    NppStatus status = nppiAddDeviceC_8u_C3RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, nppStreamCtx);
    ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiAddDeviceCTest, AddDeviceC_ErrorHandling) {
    int step;
    ...
    EXPECT_EQ(nppiAddDeviceC_8u_C1RSfs_Ctx(d_valid, step, d_constant, d_valid, step, oSizeROI, -1, nppStreamCtx), NPP_BAD_ARGUMENT_ERROR);
    EXPECT_EQ(nppiAddDeviceC_8u_C1RSfs_Ctx(d_valid, step, d_constant, d_valid, step, oSizeROI, 32, nppStreamCtx), NPP_BAD_ARGUMENT_ERROR);

    // Test null pointer errors
    EXPECT_EQ(nppiAddDeviceC_8u_C1RSfs_Ctx(nullptr, step, d_constant, d_valid, step, oSizeROI, 0, nppStreamCtx), NPP_NULL_POINTER_ERROR);
    EXPECT_EQ(nppiAddDeviceC_8u_C1RSfs_Ctx(d_valid, step, nullptr, d_valid, step, oSizeROI, 0, nppStreamCtx), NPP_NULL_POINTER_ERROR);
    EXPECT_EQ(nppiAddDeviceC_8u_C1RSfs_Ctx(d_valid, step, d_constant, nullptr, step, oSizeROI, 0, nppStreamCtx), NPP_NULL_POINTER_ERROR);
