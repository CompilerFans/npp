...
TEST_F(ExpFunctionalTest, Exp_16s_C1RSfs_BasicOperation) {
  ...
  for (int i = 0; i < width * height; i++) {
    ...
    EXPECT_LE(diff, 5) << "Mismatch at pixel " << i << ": got " << resultData[i] << ", expected approximately "
                       << expectedData[i] << ", src=" << srcData[i];
  }
...
TEST_F(ExpFunctionalTest, Exp_32f_C1IR_InPlace) {
  ...
  // Verify results
  for (int i = 0; i < width * height; i++) {
    EXPECT_NEAR(resultData[i], expectedData[i], expectedData[i] * 1e-5f) << "In-place operation failed at pixel " << i;
  }
...
TEST_F(ExpFunctionalTest, Exp_8u_C1RSfs_WithScaling) {
  ...
  for (int i = 0; i < width * height; i++) {
    int diff = std::abs((int)resultData[i] - (int)expectedData[i]);
    EXPECT_LE(diff, 10) << "Mismatch at pixel " << i << ": got " << (int)resultData[i] << ", expected approximately "
