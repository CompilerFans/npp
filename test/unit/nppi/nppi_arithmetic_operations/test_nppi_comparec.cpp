...
protected:
    ...
    // 辅助函数：验证CompareC结果
    template<typename T>
    void verifyCompareC(const std::vector<T>& src, const std::vector<Npp8u>& result, 
                       T constant, NppCmpOp operation) {
        for (size_t i = 0; i < result.size(); i++) {
            ...
            EXPECT_EQ(result[i], expectedValue) << "Mismatch at index " << i 
                      << " src=" << (int)src[i] << " const=" << (int)constant 
                      << " op=" << operation;
        }
    }
};
