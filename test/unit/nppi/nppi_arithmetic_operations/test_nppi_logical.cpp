...
protected:
    void SetUp() override {
        ...
        // 上传到设备
        cudaError_t err1 = cudaMemcpy2D(d_src1, step1, h_src1.data(), width,
                                        width, height, cudaMemcpyHostToDevice);
        cudaError_t err2 = cudaMemcpy2D(d_src2, step2, h_src2.data(), width,
                                        width, height, cudaMemcpyHostToDevice);
    ...
    void TearDown() override {
        // 使用NPP内置函数释放内存
        if (d_src1) nppiFree(d_src1);
        if (d_src2) nppiFree(d_src2);
...
TEST_F(LogicalFunctionalTest, And_8u_C1R) {
    // 执行AND操作
    NppStatus status = nppiAnd_8u_C1R(d_src1, step1, d_src2, step2, 
                                       d_dst, step_dst, size);
    ASSERT_EQ(status, NPP_SUCCESS);

    // 下载结果
    cudaError_t err = cudaMemcpy2D(h_dst.data(), width, d_dst, step_dst,
...
TEST_F(LogicalFunctionalTest, AndC_8u_C1R) {
    ...
    // 执行AND常数操作
    NppStatus status = nppiAndC_8u_C1R(d_src1, step1, constant, 
                                       d_dst, step_dst, size);
    ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(LogicalFunctionalTest, Or_8u_C1R) {
    // 执行OR操作
    NppStatus status = nppiOr_8u_C1R(d_src1, step1, d_src2, step2, 
                                      d_dst, step_dst, size);
    ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(LogicalFunctionalTest, OrC_8u_C1R) {
    ...
    // 执行OR常数操作
    NppStatus status = nppiOrC_8u_C1R(d_src1, step1, constant, 
                                      d_dst, step_dst, size);
    ASSERT_EQ(status, NPP_SUCCESS);
