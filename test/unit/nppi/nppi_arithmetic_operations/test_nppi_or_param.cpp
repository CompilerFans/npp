...
class OrC8uC3ParamTest : public NppTestBase, public ::testing::WithParamInterface<OrC8uParam> {};

TEST_P(OrC8uC3ParamTest, OrC_8u_C3R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  const Npp8u constant = param.constant;
  ...
  std::vector<Npp8u> expectedData(total);
  for (size_t i = 0; i < expectedData.size(); i++) {
    expectedData[i] = expect::or_c<Npp8u>(srcData[i], aConstants[i % 3]);
  }
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOrC_8u_C3IR_Ctx(aConstants, src.get(), src.step(), roi, ctx);
    } else {
      status = nppiOrC_8u_C3IR(aConstants, src.get(), src.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOrC_8u_C3R_Ctx(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiOrC_8u_C3R(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(OrC8uC3, OrC8uC3ParamTest,
                         ::testing::Values(OrC8uParam{32, 32, 0xF0, false, false, "32x32_cF0_noCtx"},
                                           OrC8uParam{32, 32, 0xF0, true, false, "32x32_cF0_Ctx"},
                                           OrC8uParam{32, 32, 0x0F, false, true, "32x32_c0F_InPlace"},
                                           OrC8uParam{32, 32, 0x0F, true, true, "32x32_c0F_InPlace_Ctx"}),
...
class OrC8uC4ParamTest : public NppTestBase, public ::testing::WithParamInterface<OrC8uParam> {};

TEST_P(OrC8uC4ParamTest, OrC_8u_C4R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  const Npp8u constant = param.constant;
  ...
  std::vector<Npp8u> expectedData(total);
  for (size_t i = 0; i < expectedData.size(); i++) {
    expectedData[i] = expect::or_c<Npp8u>(srcData[i], aConstants[i % 4]);
  }
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOrC_8u_C4IR_Ctx(aConstants, src.get(), src.step(), roi, ctx);
    } else {
      status = nppiOrC_8u_C4IR(aConstants, src.get(), src.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOrC_8u_C4R_Ctx(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiOrC_8u_C4R(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(OrC8uC4, OrC8uC4ParamTest,
                         ::testing::Values(OrC8uParam{32, 32, 0xF0, false, false, "32x32_cF0_noCtx"},
                                           OrC8uParam{32, 32, 0xF0, true, false, "32x32_cF0_Ctx"},
                                           OrC8uParam{32, 32, 0x0F, false, true, "32x32_c0F_InPlace"},
                                           OrC8uParam{32, 32, 0x0F, true, true, "32x32_c0F_InPlace_Ctx"}),
...
class Or16uC1ParamTest : public NppTestBase, public ::testing::WithParamInterface<Or16uParam> {};

TEST_P(Or16uC1ParamTest, Or_16u_C1R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOr_16u_C1IR_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), roi, ctx);
    } else {
      status = nppiOr_16u_C1IR(src1.get(), src1.step(), src2.get(), src2.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOr_16u_C1R_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiOr_16u_C1R(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(Or16uC1, Or16uC1ParamTest,
                         ::testing::Values(Or16uParam{32, 32, false, false, "32x32_noCtx"},
                                           Or16uParam{32, 32, true, false, "32x32_Ctx"},
                                           Or16uParam{32, 32, false, true, "32x32_InPlace"},
                                           Or16uParam{32, 32, true, true, "32x32_InPlace_Ctx"}),
                         [](const ::testing::TestParamInfo<Or16uParam> &info) { return info.param.name; });

// ==================== Or 16u C3 TEST_P ====================

class Or16uC3ParamTest : public NppTestBase, public ::testing::WithParamInterface<Or16uParam> {};

TEST_P(Or16uC3ParamTest, Or_16u_C3R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  const int channels = 3;
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOr_16u_C3IR_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), roi, ctx);
    } else {
      status = nppiOr_16u_C3IR(src1.get(), src1.step(), src2.get(), src2.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiOr_16u_C3R_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiOr_16u_C3R(src1.get(), src1.step(), src2.get(), src2.step(), dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(Or16uC3, Or16uC3ParamTest,
                         ::testing::Values(Or16uParam{32, 32, false, false, "32x32_noCtx"},
                                           Or16uParam{32, 32, true, false, "32x32_Ctx"},
                                           Or16uParam{32, 32, false, true, "32x32_InPlace"},
                                           Or16uParam{32, 32, true, true, "32x32_InPlace_Ctx"}),
