...
class Ln32fC3ParamTest : public NppTestBase, public ::testing::WithParamInterface<Ln32fParam> {};

TEST_P(Ln32fC3ParamTest, Ln_32f_C3R) {
  const auto &param = GetParam();
  const int width = param.width;
  const int height = param.height;
  ...
  if (param.in_place) {
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiLn_32f_C3IR_Ctx(src.get(), src.step(), roi, ctx);
    } else {
      status = nppiLn_32f_C3IR(src.get(), src.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
    ...
    if (param.use_ctx) {
      NppStreamContext ctx{};
      ctx.hStream = 0;
      status = nppiLn_32f_C3R_Ctx(src.get(), src.step(), dst.get(), dst.step(), roi, ctx);
    } else {
      status = nppiLn_32f_C3R(src.get(), src.step(), dst.get(), dst.step(), roi);
    }
    ASSERT_EQ(status, NPP_NO_ERROR);
...
INSTANTIATE_TEST_SUITE_P(Ln32fC3, Ln32fC3ParamTest,
                         ::testing::Values(Ln32fParam{32, 32, false, false, "32x32_noCtx"},
                                           Ln32fParam{32, 32, true, false, "32x32_Ctx"},
                                           Ln32fParam{32, 32, false, true, "32x32_InPlace"},
                                           Ln32fParam{32, 32, true, true, "32x32_InPlace_Ctx"}),
