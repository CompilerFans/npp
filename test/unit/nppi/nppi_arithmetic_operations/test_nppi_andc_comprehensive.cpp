...
protected:
  ...
  template <typename T>
  void computeAndCReference(const std::vector<T> &src, const std::vector<T> &constants, 
                           std::vector<T> &dst, int channels) {
    dst.resize(src.size());
  ...
  template <typename T>
  std::vector<T> generateConstantData(int width, int height, int channels, T value) {
    return std::vector<T>(width * height * channels, value);
  }

  template <typename T>
  std::vector<T> generatePatternData(int width, int height, int channels) {
    std::vector<T> data(width * height * channels);
...
TEST_F(NppiAndCComprehensiveTest, AndC_8u_C3R_Ctx_Comprehensive) {
  ...
  testCases.push_back({
    "Constant_128", 
    generateConstantData<Npp8u>(width, height, channels, 128),
    {0xFF, 0x0F, 0xF0}
  });
  ...
  testCases.push_back({
    "Random_Data", 
    generateTestData<Npp8u>(width, height, channels, 0, 255),
    {0xAA, 0x55, 0xCC}
  });
  ...
  testCases.push_back({
    "Pattern_Data", 
    generatePatternData<Npp8u>(width, height, channels),
    {0x3F, 0x7F, 0x1F}
  });
  ...
  testCases.push_back({
    "Extreme_Values", 
    generateTestData<Npp8u>(width, height, channels, 0, 255, 123),
    {0x00, 0xFF, 0x80}
  });
  ...
  for (const auto &testCase : testCases) {
    ...
    // Test Context version
    {
      ...
      NppStatus status = nppiAndC_8u_C3R_Ctx(d_src, srcStep, testCase.constants.data(), 
                                             d_dst, dstStep, oSizeROI, nppStreamCtx);
    ...
    // Test non-context version
    {
      ...
      NppStatus status = nppiAndC_8u_C3R(d_src, srcStep, testCase.constants.data(), 
                                        d_dst, dstStep, oSizeROI);
      ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiAndCComprehensiveTest, AndC_16u_C3R_Ctx_Comprehensive) {
  ...
  NppStatus status = nppiAndC_16u_C3R_Ctx(d_src, srcStep, constants.data(), 
                                         d_dst, dstStep, oSizeROI, nppStreamCtx);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiAndCComprehensiveTest, AndC_16u_C4R_Ctx_Comprehensive) {
  ...
  NppStatus status = nppiAndC_16u_C4R_Ctx(d_src, srcStep, constants.data(), 
                                         d_dst, dstStep, oSizeROI, nppStreamCtx);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiAndCComprehensiveTest, AndC_32s_C3R_Ctx_Comprehensive) {
  ...
  NppStreamContext nppStreamCtx;
  memset(&nppStreamCtx, 0, sizeof(NppStreamContext));
  NppStatus status = nppiAndC_32s_C3R_Ctx(d_src, srcStep, constants.data(), 
                                         d_dst, dstStep, oSizeROI, nppStreamCtx);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiAndCComprehensiveTest, AndC_32s_C4R_Ctx_Comprehensive) {
  ...
  std::vector<Npp32s> constants = {0x12345678, static_cast<Npp32s>(0x87654321), static_cast<Npp32s>(0xAAAAAAAA), 0x55555555};
  ...
  NppStreamContext nppStreamCtx;
  memset(&nppStreamCtx, 0, sizeof(NppStreamContext));
  NppStatus status = nppiAndC_32s_C4R_Ctx(d_src, srcStep, constants.data(), 
                                         d_dst, dstStep, oSizeROI, nppStreamCtx);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiAndCComprehensiveTest, ErrorHandling) {
  ...
  // Test size errors
  EXPECT_EQ(nppiAndC_8u_C3R_Ctx(d_src, srcStep, constants, d_dst, dstStep, {-1, height}, nppStreamCtx), 
            NPP_SIZE_ERROR);
  EXPECT_EQ(nppiAndC_8u_C3R_Ctx(d_src, srcStep, constants, d_dst, dstStep, {width, -1}, nppStreamCtx), 
            NPP_SIZE_ERROR);
...
TEST_F(NppiAndCComprehensiveTest, PerformanceComparison) {
  ...
  for (int i = 0; i < iterations; i++) {
    NppStatus status = nppiAndC_8u_C3R_Ctx(d_src, srcStep, constants.data(), 
                                           d_dst, dstStep, oSizeROI, nppStreamCtx);
    ASSERT_EQ(status, NPP_SUCCESS);
  }
  ...
  for (int i = 0; i < iterations; i++) {
    NppStatus status = nppiAndC_8u_C3R(d_src, srcStep, constants.data(), 
                                      d_dst, dstStep, oSizeROI);
    ASSERT_EQ(status, NPP_SUCCESS);
  }
  cudaDeviceSynchronize();
