...
TEST_F(SqrtFunctionalTest, Sqrt_32f_C1R_BasicOperation) {
  ...
  for (int i = 0; i < width * height; i++) {
    EXPECT_NEAR(resultData[i], expectedData[i], 1e-6f) << "Mismatch at pixel " << i << ": got " << resultData[i]
...
TEST_F(SqrtFunctionalTest, Sqrt_16s_C1RSfs_BasicOperation) {
  ...
  // Verify results
  for (int i = 0; i < width * height; i++) {
    EXPECT_EQ(resultData[i], expectedData[i]) << "Mismatch at pixel " << i << ": got " << resultData[i] << ", expected "
...
TEST_F(SqrtFunctionalTest, Sqrt_StreamContext) {
  ...
  for (int i = 0; i < width * height; i++) {
    EXPECT_NEAR(resultData[i], 3.0f, 1e-6f) << "Stream context test failed at pixel " << i;
  }
