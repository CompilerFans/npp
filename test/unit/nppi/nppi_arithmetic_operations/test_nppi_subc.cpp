...
// ==================== SubC 16f (half-precision float) tests ====================

class SubC16fTest : public NppTestBase {};

TEST_F(SubC16fTest, SubC_16f_C1R_BasicOperation) {
  const int width = 32;
  const int height = 32;
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiSubC_16f_C1R(src.get(), src.step(), constant, dst.get(), dst.step(), roi);

  ASSERT_EQ(status, NPP_NO_ERROR) << "nppiSubC_16f_C1R failed";

  std::vector<Npp16f> resultData(width * height);
  dst.copyToHost(resultData);

  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "SubC 16f C1R operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C1R_Ctx_BasicOperation) {
  const int width = 32;
  const int height = 32;
  const Npp32f constant = 10.5f;
  ...
  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "SubC 16f C1R Ctx operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C1IR_InPlaceOperation) {
  const int width = 32;
  const int height = 32;
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiSubC_16f_C1IR(constant, srcDst.get(), srcDst.step(), roi);

  ASSERT_EQ(status, NPP_NO_ERROR) << "nppiSubC_16f_C1IR failed";

  std::vector<Npp16f> resultData(width * height);
  srcDst.copyToHost(resultData);

  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "In-place SubC 16f C1 operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C1IR_Ctx_InPlaceOperation) {
  const int width = 32;
  const int height = 32;
  const Npp32f constant = 15.0f;
  ...
  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "In-place SubC 16f C1 Ctx operation produced incorrect results";
}

// 16f C3 tests
TEST_F(SubC16fTest, SubC_16f_C3R_BasicOperation) {
  const int width = 32;
  const int height = 32;
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiSubC_16f_C3R(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi);

  ASSERT_EQ(status, NPP_NO_ERROR) << "nppiSubC_16f_C3R failed";

  std::vector<Npp16f> resultData(width * height * channels);
  dst.copyToHost(resultData);

  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "SubC 16f C3R operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C3R_Ctx_BasicOperation) {
  const int width = 32;
  const int height = 32;
  const int channels = 3;
  const Npp32f aConstants[3] = {5.0f, 10.0f, 15.0f};
  ...
  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "SubC 16f C3R Ctx operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C3IR_InPlaceOperation) {
  const int width = 32;
  const int height = 32;
  const int channels = 3;
  const Npp32f aConstants[3] = {5.0f, 10.0f, 15.0f};
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiSubC_16f_C3IR(aConstants, srcDst.get(), srcDst.step(), roi);

  ASSERT_EQ(status, NPP_NO_ERROR) << "nppiSubC_16f_C3IR failed";

  std::vector<Npp16f> resultData(width * height * channels);
  srcDst.copyToHost(resultData);

  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "In-place SubC 16f C3 operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C3IR_Ctx_InPlaceOperation) {
  const int width = 32;
  const int height = 32;
  const int channels = 3;
  const Npp32f aConstants[3] = {5.0f, 10.0f, 15.0f};
  ...
  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "In-place SubC 16f C3 Ctx operation produced incorrect results";
}

// 16f C4 tests
TEST_F(SubC16fTest, SubC_16f_C4R_BasicOperation) {
  const int width = 32;
  const int height = 32;
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiSubC_16f_C4R(src.get(), src.step(), aConstants, dst.get(), dst.step(), roi);

  ASSERT_EQ(status, NPP_NO_ERROR) << "nppiSubC_16f_C4R failed";

  std::vector<Npp16f> resultData(width * height * channels);
  dst.copyToHost(resultData);

  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "SubC 16f C4R operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C4R_Ctx_BasicOperation) {
  const int width = 32;
  const int height = 32;
  const int channels = 4;
  const Npp32f aConstants[4] = {5.0f, 10.0f, 15.0f, 20.0f};
  ...
  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "SubC 16f C4R Ctx operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C4IR_InPlaceOperation) {
  const int width = 32;
  const int height = 32;
  const int channels = 4;
  const Npp32f aConstants[4] = {5.0f, 10.0f, 15.0f, 20.0f};
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiSubC_16f_C4IR(aConstants, srcDst.get(), srcDst.step(), roi);

  ASSERT_EQ(status, NPP_NO_ERROR) << "nppiSubC_16f_C4IR failed";

  std::vector<Npp16f> resultData(width * height * channels);
  srcDst.copyToHost(resultData);

  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "In-place SubC 16f C4 operation produced incorrect results";
}

TEST_F(SubC16fTest, SubC_16f_C4IR_Ctx_InPlaceOperation) {
  const int width = 32;
  const int height = 32;
  const int channels = 4;
  const Npp32f aConstants[4] = {5.0f, 10.0f, 15.0f, 20.0f};
  ...
  EXPECT_TRUE(ResultValidator::arraysEqual16f(resultData, expectedData, 1e-2f))
      << "In-place SubC 16f C4 Ctx operation produced incorrect results";
}
