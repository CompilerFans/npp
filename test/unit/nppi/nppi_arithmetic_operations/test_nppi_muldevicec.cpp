...
TEST_F(NppiMulDeviceCTest, MulDeviceC_8u_C1RSfs_WithScaling) {
  ...
  nppGetStreamContext(&nppStreamCtx);
  NppStatus status =
      nppiMulDeviceC_8u_C1RSfs_Ctx(d_src, step, d_constant, d_dst, step, oSizeROI, scaleFactor, nppStreamCtx);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(NppiMulDeviceCTest, MulDeviceC_8u_C3RSfs_MultiChannel) {
  ...
  // Expected results
  std::vector<Npp8u> expected = {// Pixel (0,0): R=10*2=20, G=20*3=60, B=30*4=120
                                 20, 60, 120,
  ...
  nppGetStreamContext(&nppStreamCtx);
  NppStatus status =
      nppiMulDeviceC_8u_C3RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, nppStreamCtx);
  ASSERT_EQ(status, NPP_SUCCESS);
