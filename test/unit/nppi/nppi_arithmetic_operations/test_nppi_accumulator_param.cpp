...
static const std::vector<AccumulatorParam> kAccumulatorParams = {
    {32, 32, false, false}, {32, 32, true, false}, {64, 64, false, false},
    {64, 64, true, false},  {32, 32, false, true}, {32, 32, true, true},
};
...
TEST_P(AddProduct8u32fParamTest, AddProduct_8u32f_C1IR) {
  ...
  if (p.use_mask) {
    ...
    status = p.use_ctx ? nppiAddProduct_8u32f_C1IMR_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), mask.get(),
                                                        mask.step(), acc.get(), acc.step(), roi, ctx)
                       : nppiAddProduct_8u32f_C1IMR(src1.get(), src1.step(), src2.get(), src2.step(), mask.get(),
                                                    mask.step(), acc.get(), acc.step(), roi);
  } else {
    status = p.use_ctx ? nppiAddProduct_8u32f_C1IR_Ctx(src1.get(), src1.step(), src2.get(), src2.step(), acc.get(),
                                                       acc.step(), roi, ctx)
                       : nppiAddProduct_8u32f_C1IR(src1.get(), src1.step(), src2.get(), src2.step(), acc.get(),
                                                   acc.step(), roi);
  }
...
TEST_P(AddProduct16u32fParamTest, AddProduct_16u32f_C1IR) {
  const auto &p = GetParam();
  if (p.use_mask)
    return; // 16u version doesn't have mask variant
  const int total = p.width * p.height;
  ...
  NppStreamContext ctx;
  nppGetStreamContext(&ctx);
  NppStatus status = p.use_ctx ? nppiAddProduct_16u32f_C1IR_Ctx(src1.get(), src1.step(), src2.get(), src2.step(),
                                                                acc.get(), acc.step(), roi, ctx)
                               : nppiAddProduct_16u32f_C1IR(src1.get(), src1.step(), src2.get(), src2.step(), acc.get(),
                                                            acc.step(), roi);
  ASSERT_EQ(status, NPP_NO_ERROR);
...
TEST_P(AddSquare8u32fParamTest, AddSquare_8u32f_C1IR) {
  ...
  if (p.use_mask) {
    NppImageMemory<Npp8u> mask(p.width, p.height);
    mask.copyFromHost(maskData);
    status = p.use_ctx ? nppiAddSquare_8u32f_C1IMR_Ctx(src.get(), src.step(), mask.get(), mask.step(), acc.get(),
                                                       acc.step(), roi, ctx)
                       : nppiAddSquare_8u32f_C1IMR(src.get(), src.step(), mask.get(), mask.step(), acc.get(),
                                                   acc.step(), roi);
  } else {
    status = p.use_ctx ? nppiAddSquare_8u32f_C1IR_Ctx(src.get(), src.step(), acc.get(), acc.step(), roi, ctx)
...
TEST_P(AddSquare16u32fParamTest, AddSquare_16u32f_C1IR) {
  ...
  NppStreamContext ctx;
  nppGetStreamContext(&ctx);
  NppStatus status = p.use_ctx ? nppiAddSquare_16u32f_C1IR_Ctx(src.get(), src.step(), acc.get(), acc.step(), roi, ctx)
...
static const std::vector<AddWeightedParam> kAddWeightedParams = {
    {32, 32, 0.5f, false, false}, {32, 32, 0.5f, true, false}, {32, 32, 0.25f, false, false},
    {32, 32, 0.75f, true, false}, {32, 32, 0.5f, false, true}, {32, 32, 0.5f, true, true},
};
...
TEST_P(AddWeighted8u32fParamTest, AddWeighted_8u32f_C1IR) {
  ...
  if (p.use_mask) {
    NppImageMemory<Npp8u> mask(p.width, p.height);
    mask.copyFromHost(maskData);
    status = p.use_ctx ? nppiAddWeighted_8u32f_C1IMR_Ctx(src.get(), src.step(), mask.get(), mask.step(), acc.get(),
                                                         acc.step(), roi, p.alpha, ctx)
                       : nppiAddWeighted_8u32f_C1IMR(src.get(), src.step(), mask.get(), mask.step(), acc.get(),
                                                     acc.step(), roi, p.alpha);
  } else {
    status = p.use_ctx ? nppiAddWeighted_8u32f_C1IR_Ctx(src.get(), src.step(), acc.get(), acc.step(), roi, p.alpha, ctx)
...
TEST_P(AddWeighted16u32fParamTest, AddWeighted_16u32f_C1IR) {
  ...
  nppGetStreamContext(&ctx);
  NppStatus status =
      p.use_ctx ? nppiAddWeighted_16u32f_C1IR_Ctx(src.get(), src.step(), acc.get(), acc.step(), roi, p.alpha, ctx)
...
TEST_P(AddWeighted32fParamTest, AddWeighted_32f_C1IR) {
  const auto &p = GetParam();
  if (p.use_mask)
    return; // 32f version doesn't have mask variant
  const int total = p.width * p.height;
