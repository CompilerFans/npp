...
TEST_F(MulScaleFunctionalTest, MulScale_8u_C1R_BasicOperation) {
  ...
  for (int y = 0; y < height; y++) {
    cudaMemcpy((char *)d_src1 + y * src1Step, src1Data.data() + y * width, width * sizeof(Npp8u),
               cudaMemcpyHostToDevice);
    cudaMemcpy((char *)d_src2 + y * src2Step, src2Data.data() + y * width, width * sizeof(Npp8u),
               cudaMemcpyHostToDevice);
  }
  ...
  for (int i = 0; i < width * height; i++) {
    NPP_EXPECT_ARITHMETIC_EQUAL(resultData[i], expectedData[i], "nppiMulScale_8u_C1R")
        ...
        << ", src1=" << (int)src1Data[i] << ", src2=" << (int)src2Data[i];
  }
...
TEST_F(MulScaleFunctionalTest, MulScale_16u_C1R_BasicOperation) {
  ...
  // Copy input data to GPU row by row
  for (int y = 0; y < height; y++) {
    cudaMemcpy((char *)d_src1 + y * src1Step, src1Data.data() + y * width, width * sizeof(Npp16u),
               cudaMemcpyHostToDevice);
    cudaMemcpy((char *)d_src2 + y * src2Step, src2Data.data() + y * width, width * sizeof(Npp16u),
               cudaMemcpyHostToDevice);
  }
  ...
  for (int y = 0; y < height; y++) {
    cudaMemcpy(resultData.data() + y * width, (char *)d_dst + y * dstStep, width * sizeof(Npp16u),
               cudaMemcpyDeviceToHost);
  }
  ...
  for (int i = 0; i < width * height; i++) {
    EXPECT_EQ(resultData[i], expectedData[i])
        << "Mismatch at pixel " << i << ": got " << resultData[i] << ", expected " << expectedData[i]
        << ", src1=" << src1Data[i] << ", src2=" << src2Data[i];
  }
...
TEST_F(MulScaleFunctionalTest, MulScale_8u_C1R_ExtremeValues) {
  ...
  for (int i = 0; i < testSize; i++) {
    EXPECT_EQ(resultData[i], expectedData[i])
        << "Extreme value test failed at index " << i << ", src1=" << (int)src1Data[i] << ", src2=" << (int)src2Data[i];
  }
...
TEST_F(MulScaleFunctionalTest, MulScale_8u_C1IR_InPlace) {
  ...
  for (int y = 0; y < height; y++) {
    ...
    cudaMemcpy((char *)d_srcDst + y * srcDstStep, srcDstData.data() + y * width, width * sizeof(Npp8u),
               cudaMemcpyHostToDevice);
  }
  ...
  std::vector<Npp8u> resultData(width * height);
  for (int y = 0; y < height; y++) {
    cudaMemcpy(resultData.data() + y * width, (char *)d_srcDst + y * srcDstStep, width * sizeof(Npp8u),
               cudaMemcpyDeviceToHost);
  }

  // Verify results
  for (int i = 0; i < width * height; i++) {
    EXPECT_EQ(resultData[i], expectedData[i]) << "In-place operation failed at pixel " << i;
  }
...
TEST_F(MulScaleFunctionalTest, MulScale_StreamContext) {
  ...
  for (int i = 0; i < width * height; i++) {
    EXPECT_EQ(resultData[i], 128) << "Stream context test failed at pixel " << i;
  }
...
TEST_F(MulScaleFunctionalTest, MulScale_16u_C1R_OverflowHandling) {
  ...
  for (int i = 0; i < testSize; i++) {
    EXPECT_EQ(resultData[i], expectedData[i])
        << "Overflow test failed at index " << i << ", src1=" << src1Data[i] << ", src2=" << src2Data[i];
  }
