...
template <typename T> class NppiSubTemplatedTest : public ::testing::Test {
protected:
  void SetUp() override {
  ...
  // Reference implementation for sub operation
  template <int Channels>
  std::vector<T> calculateSubReference(const std::vector<T> &src1, const std::vector<T> &src2, int scaleFactor = 0) {
    std::vector<T> result(src1.size());

    for (size_t i = 0; i < src1.size(); ++i) {
      double val1 = static_cast<double>(src1[i]);
      double val2 = static_cast<double>(src2[i]);
      double res = val2 - val1; // NPP Sub convention: pSrc2 - pSrc1
  ...
  // Test sub operations
  template <int Channels> void testSubOperation(int scaleFactor = 0) {
    // Generate test data
    ...
    // Calculate reference
    std::vector<T> reference = calculateSubReference<Channels>(hostSrc1, hostSrc2, scaleFactor);
...
TYPED_TEST_SUITE(NppiSubTemplatedTest, TestTypes);

TYPED_TEST(NppiSubTemplatedTest, SubOperation_C1) {
  if constexpr (std::is_same_v<TypeParam, Npp8u>) {
    this->template testSubOperation<1>(1);
  }
}