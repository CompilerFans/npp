...
TEST_F(LnFunctionalTest, Ln_8u_C1RSfs_BasicOperation) {
  ...
  for (int i = 0; i < width * height; i++) {
    ...
    EXPECT_LE(diff, 1) << "Mismatch at pixel " << i << ": got " << (int)resultData[i] << ", expected "
                       << (int)expectedData[i] << ", src=" << (int)srcData[i];
  }
...
TEST_F(LnFunctionalTest, Ln_8u_C1RSfs_WithScaling) {
  ...
  for (int i = 0; i < width * height; i++) {
    int diff = std::abs((int)resultData[i] - (int)expectedData[i]);
    EXPECT_LE(diff, 2) << "Mismatch at pixel " << i << ": got " << (int)resultData[i] << ", expected "
                       << (int)expectedData[i] << ", src=" << (int)srcData[i];
  }
...
TEST_F(LnFunctionalTest, Ln_32f_C1R_BasicOperation) {
  ...
  for (int i = 0; i < width * height; i++) {
    EXPECT_NEAR(resultData[i], expectedData[i], 1e-5f) << "Mismatch at pixel " << i << ": got " << resultData[i]
                                                       << ", expected " << expectedData[i] << ", src=" << srcData[i];
  }
...
TEST_F(LnFunctionalTest, Ln_16s_C1RSfs_BasicOperation) {
  ...
  for (int y = 0; y < height; y++) {
    cudaMemcpy(resultData.data() + y * width, (char *)d_dst + y * dstStep, width * sizeof(Npp16s),
               cudaMemcpyDeviceToHost);
  }
  ...
  for (int i = 0; i < width * height; i++) {
    int diff = std::abs((int)resultData[i] - (int)expectedData[i]);
    EXPECT_LE(diff, 1) << "Mismatch at pixel " << i << ": got " << resultData[i] << ", expected " << expectedData[i]
                       << ", src=" << srcData[i];
  }
...
TEST_F(LnFunctionalTest, Ln_32f_C1IR_InPlace) {
  ...
  // Copy input data to GPU row by row
  for (int y = 0; y < height; y++) {
    cudaMemcpy((char *)d_data + y * step, data.data() + y * width, width * sizeof(Npp32f), cudaMemcpyHostToDevice);
  }
  ...
  std::vector<Npp32f> resultData(width * height);
  for (int y = 0; y < height; y++) {
    cudaMemcpy(resultData.data() + y * width, (char *)d_data + y * step, width * sizeof(Npp32f),
               cudaMemcpyDeviceToHost);
  }

  // Verify results
  for (int i = 0; i < width * height; i++) {
    EXPECT_NEAR(resultData[i], expectedData[i], 1e-5f) << "In-place operation failed at pixel " << i;
  }
...
TEST_F(LnFunctionalTest, Ln_StreamContext) {
  ...
  for (int i = 0; i < width * height; i++) {
    EXPECT_NEAR(resultData[i], 1.0f, 1e-6f) << "Stream context test failed at pixel " << i;
  }
