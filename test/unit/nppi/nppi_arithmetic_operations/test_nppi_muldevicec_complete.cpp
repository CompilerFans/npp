...
class MulDeviceC_8u_Test : public ::testing::TestWithParam<MulDeviceCParam> {
protected:
...
TEST_P(MulDeviceC_8u_Test, Compute) {
  ...
  for (int i = 0; i < totalElements; ++i) {
    hostSrc[i] = static_cast<Npp8u>(5 + (i % 20));
  }

  std::vector<Npp8u> hostConstants(channelCount);
  for (int c = 0; c < channelCount; ++c) {
    hostConstants[c] = static_cast<Npp8u>(2 + c);
  }
  ...
  if (p.inplace) {
    if (channelCount == 1) {
      status = nppiMulDeviceC_8u_C1IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceC_8u_C3IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceC_8u_AC4IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    } else {
      status = nppiMulDeviceC_8u_C4IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    }
  } else {
    if (channelCount == 1) {
      status = nppiMulDeviceC_8u_C1RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceC_8u_C3RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceC_8u_AC4RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    } else {
      status = nppiMulDeviceC_8u_C4RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    }
  }
...
class MulDeviceC_16u_Test : public ::testing::TestWithParam<MulDeviceCParam> {
protected:
  void SetUp() override { cudaSetDevice(0); }
  void TearDown() override { cudaDeviceSynchronize(); }
};
...
TEST_P(MulDeviceC_16u_Test, Compute) {
  ...
  for (int i = 0; i < totalElements; ++i) {
    hostSrc[i] = static_cast<Npp16u>(50 + (i % 200));
  }

  std::vector<Npp16u> hostConstants(channelCount);
  for (int c = 0; c < channelCount; ++c) {
    hostConstants[c] = static_cast<Npp16u>(2 + c);
  }
  ...
  if (p.inplace) {
    if (channelCount == 1) {
      status = nppiMulDeviceC_16u_C1IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceC_16u_C3IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceC_16u_AC4IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    } else {
      status = nppiMulDeviceC_16u_C4IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    }
  } else {
    if (channelCount == 1) {
      status = nppiMulDeviceC_16u_C1RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceC_16u_C3RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceC_16u_AC4RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    } else {
      status = nppiMulDeviceC_16u_C4RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    }
  }
  ASSERT_EQ(status, NPP_SUCCESS);
...
class MulDeviceC_16s_Test : public ::testing::TestWithParam<MulDeviceCParam> {
protected:
  void SetUp() override { cudaSetDevice(0); }
  void TearDown() override { cudaDeviceSynchronize(); }
};
...
TEST_P(MulDeviceC_16s_Test, Compute) {
  ...
  for (int i = 0; i < totalElements; ++i) {
    hostSrc[i] = static_cast<Npp16s>(-50 + (i % 100));
  }

  std::vector<Npp16s> hostConstants(channelCount);
  for (int c = 0; c < channelCount; ++c) {
    hostConstants[c] = static_cast<Npp16s>(2 + c);
  }
  ...
  if (p.inplace) {
    status = nppiMulDeviceC_16s_C1IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
  } else {
    status = nppiMulDeviceC_16s_C1RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
  }
  ASSERT_EQ(status, NPP_SUCCESS);
...
class MulDeviceC_32s_Test : public ::testing::TestWithParam<MulDeviceCParam> {
protected:
  void SetUp() override { cudaSetDevice(0); }
  void TearDown() override { cudaDeviceSynchronize(); }
};
...
TEST_P(MulDeviceC_32s_Test, Compute) {
  ...
  for (int i = 0; i < totalElements; ++i) {
    hostSrc[i] = static_cast<Npp32s>(-500 + (i % 1000));
  }

  std::vector<Npp32s> hostConstants(channelCount);
  for (int c = 0; c < channelCount; ++c) {
    hostConstants[c] = static_cast<Npp32s>(2 + c);
  }
  ...
  if (p.inplace) {
    if (channelCount == 1) {
      status = nppiMulDeviceC_32s_C1IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    } else {
      status = nppiMulDeviceC_32s_C3IRSfs_Ctx(d_constants, d_src, step, oSizeROI, scaleFactor, ctx);
    }
  } else {
    if (channelCount == 1) {
      status = nppiMulDeviceC_32s_C1RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    } else {
      status = nppiMulDeviceC_32s_C3RSfs_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, scaleFactor, ctx);
    }
  }
  ASSERT_EQ(status, NPP_SUCCESS);
...
class MulDeviceC_32f_Test : public ::testing::TestWithParam<MulDeviceCParam> {
protected:
  void SetUp() override { cudaSetDevice(0); }
  void TearDown() override { cudaDeviceSynchronize(); }
};
...
TEST_P(MulDeviceC_32f_Test, Compute) {
  ...
  if (p.inplace) {
    if (channelCount == 1) {
      status = nppiMulDeviceC_32f_C1IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceC_32f_C3IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceC_32f_AC4IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else {
      status = nppiMulDeviceC_32f_C4IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    }
  } else {
    if (channelCount == 1) {
      status = nppiMulDeviceC_32f_C1R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceC_32f_C3R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceC_32f_AC4R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else {
      status = nppiMulDeviceC_32f_C4R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    }
  }
  ASSERT_EQ(status, NPP_SUCCESS);
...
class MulDeviceCScale_8u_Test : public ::testing::TestWithParam<MulDeviceCParam> {
protected:
  void SetUp() override { cudaSetDevice(0); }
  void TearDown() override { cudaDeviceSynchronize(); }
};
...
TEST_P(MulDeviceCScale_8u_Test, Compute) {
  ...
  if (p.inplace) {
    if (channelCount == 1) {
      status = nppiMulDeviceCScale_8u_C1IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceCScale_8u_C3IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceCScale_8u_AC4IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else {
      status = nppiMulDeviceCScale_8u_C4IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    }
  } else {
    if (channelCount == 1) {
      status = nppiMulDeviceCScale_8u_C1R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceCScale_8u_C3R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceCScale_8u_AC4R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else {
      status = nppiMulDeviceCScale_8u_C4R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    }
  }
  ASSERT_EQ(status, NPP_SUCCESS);
  ...
  for (int i = 0; i < totalElements; ++i) {
    ...
    if (p.channels == -4 && ch == 3) {
      ...
      // MulScale: result = (src * constant) / 255
      int expected = (static_cast<int>(hostSrc[i]) * static_cast<int>(hostConstants[ch])) / 255;
      expected = std::min(255, std::max(0, expected));
      EXPECT_NEAR(hostResult[i], expected, 1) << "Mismatch at index " << i;
    }
  }
...
class MulDeviceCScale_16u_Test : public ::testing::TestWithParam<MulDeviceCParam> {
protected:
  void SetUp() override { cudaSetDevice(0); }
  void TearDown() override { cudaDeviceSynchronize(); }
};
...
TEST_P(MulDeviceCScale_16u_Test, Compute) {
  ...
  std::vector<Npp16u> hostConstants(channelCount);
  for (int c = 0; c < channelCount; ++c) {
    hostConstants[c] = static_cast<Npp16u>(32768 + c * 8192);
  }
  ...
  if (p.inplace) {
    if (channelCount == 1) {
      status = nppiMulDeviceCScale_16u_C1IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceCScale_16u_C3IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceCScale_16u_AC4IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    } else {
      status = nppiMulDeviceCScale_16u_C4IR_Ctx(d_constants, d_src, step, oSizeROI, ctx);
    }
  } else {
    if (channelCount == 1) {
      status = nppiMulDeviceCScale_16u_C1R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else if (channelCount == 3) {
      status = nppiMulDeviceCScale_16u_C3R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else if (p.channels == -4) {
      status = nppiMulDeviceCScale_16u_AC4R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    } else {
      status = nppiMulDeviceCScale_16u_C4R_Ctx(d_src, step, d_constants, d_dst, step, oSizeROI, ctx);
    }
  }
  ASSERT_EQ(status, NPP_SUCCESS);
  ...
  for (int i = 0; i < totalElements; ++i) {
    ...
    if (p.channels == -4 && ch == 3) {
      ...
      // MulScale: result = (src * constant) / 65535
      int64_t expected = (static_cast<int64_t>(hostSrc[i]) * static_cast<int64_t>(hostConstants[ch])) / 65535;
      expected = std::min((int64_t)65535, std::max((int64_t)0, expected));
      EXPECT_NEAR(hostResult[i], expected, 1) << "Mismatch at index " << i;
    }
  }
