...
TEST_F(HistogramEvenMultiChannelTest, HistogramEven_8u_C4R_Basic) {
  ...
  EXPECT_EQ(nppiHistogramEven_8u_C4R(d_src_8u_c4, nSrcStep_8u_c4, oSizeROI, d_hist, nLevels, nLowerLevel, nUpperLevel,
                                     d_buffer),
            NPP_SUCCESS);

  // Copy results back to host for each channel
  for (int c = 0; c < 4; c++) {
    ASSERT_EQ(cudaMemcpy(h_hist[c].data(), d_hist[c], (nLevels[c] - 1) * sizeof(Npp32s), cudaMemcpyDeviceToHost),
              cudaSuccess);
  }
...
TEST_F(HistogramEvenMultiChannelTest, HistogramEven_8u_C4R_WithContext) {
  ...
  EXPECT_EQ(nppiHistogramEven_8u_C4R_Ctx(d_src_8u_c4, nSrcStep_8u_c4, oSizeROI, d_hist, nLevels, nLowerLevel,
                                         nUpperLevel, d_buffer, nppStreamCtx),
            NPP_SUCCESS);
...
TEST_F(HistogramEvenMultiChannelTest, DifferentLevelsPerChannel) {
  ...
  for (int c = 0; c < 4; c++) {
    ...
    EXPECT_GT(totalCount, totalPixels * 0.95)
        << "Channel " << c << " with " << nLevels[c] << " levels should have most pixels counted";
    EXPECT_LE(totalCount, totalPixels) << "Channel " << c << " with " << nLevels[c]
                                       << " levels should not exceed total pixels";
  }
}
