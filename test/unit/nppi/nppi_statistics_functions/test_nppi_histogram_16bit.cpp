...
protected:
    ...
    void allocateTestData16u() {
        ...
        ASSERT_EQ(cudaMemcpy(d_src_16u, h_src_16u.data(), totalPixels * sizeof(Npp16u), 
    ...
    void allocateTestData16s() {
        ...
        ASSERT_EQ(cudaMemcpy(d_src_16s, h_src_16s.data(), totalPixels * sizeof(Npp16s), 
                           cudaMemcpyHostToDevice), cudaSuccess);
    }
...
TEST_F(HistogramEven16BitTest, BufferSize_16u_C1R_Basic) {
    ...
    EXPECT_EQ(nppiHistogramEvenGetBufferSize_16u_C1R_Ctx(oSizeROI, nLevels, &bufferSize, 
                                                         NppStreamContext{}), NPP_SUCCESS);
    EXPECT_GT(bufferSize, 0);
}
...
TEST_F(HistogramEven16BitTest, BufferSize_16s_C1R_Basic) {
    ...
    EXPECT_EQ(nppiHistogramEvenGetBufferSize_16s_C1R_Ctx(oSizeROI, nLevels, &bufferSize, 
                                                         NppStreamContext{}), NPP_SUCCESS);
    EXPECT_GT(bufferSize, 0);
}
...
TEST_F(HistogramEven16BitTest, HistogramEven_16u_C1R_Basic) {
    ...
    // Compute histogram
    EXPECT_EQ(nppiHistogramEven_16u_C1R(d_src_16u, nSrcStep_16u, oSizeROI, d_hist, nLevels,
                                       nLowerLevel, nUpperLevel_16u, d_buffer), NPP_SUCCESS);
...
TEST_F(HistogramEven16BitTest, HistogramEven_16s_C1R_Basic) {
    ...
    EXPECT_EQ(nppiHistogramEven_16s_C1R(d_src_16s, nSrcStep_16s, oSizeROI, d_hist, nLevels,
                                       testLower, testUpper, d_buffer), NPP_SUCCESS);
...
TEST_F(HistogramEven16BitTest, HistogramEven_16u_C1R_WithContext) {
    ...
    EXPECT_EQ(nppiHistogramEven_16u_C1R_Ctx(d_src_16u, nSrcStep_16u, oSizeROI, d_hist, nLevels,
                                           nLowerLevel, nUpperLevel_16u, d_buffer, nppStreamCtx), 
              NPP_SUCCESS);
...
TEST_F(HistogramEven16BitTest, ParameterValidation_16u) {
    ...
    EXPECT_EQ(nppiHistogramEven_16u_C1R(nullptr, nSrcStep_16u, oSizeROI, d_hist, nLevels,
                                       nLowerLevel, nUpperLevel_16u, d_buffer), 
              NPP_NULL_POINTER_ERROR);
    
    EXPECT_EQ(nppiHistogramEven_16u_C1R(d_src_16u, nSrcStep_16u, oSizeROI, nullptr, nLevels,
                                       nLowerLevel, nUpperLevel_16u, d_buffer), 
              NPP_NULL_POINTER_ERROR);
    
    // Test size error
    NppiSize invalidSize = {0, height};
    EXPECT_EQ(nppiHistogramEven_16u_C1R(d_src_16u, nSrcStep_16u, invalidSize, d_hist, nLevels,
                                       nLowerLevel, nUpperLevel_16u, d_buffer), 
              NPP_SIZE_ERROR);
    
    // Test range error
    EXPECT_EQ(nppiHistogramEven_16u_C1R(d_src_16u, nSrcStep_16u, oSizeROI, d_hist, nLevels,
                                       nUpperLevel_16u, nLowerLevel, d_buffer), 
              NPP_RANGE_ERROR);
}
...
TEST_F(HistogramEven16BitTest, ParameterValidation_16s) {
    ...
    // Test null pointer errors
    EXPECT_EQ(nppiHistogramEven_16s_C1R(nullptr, nSrcStep_16s, oSizeROI, d_hist, nLevels,
                                       nLowerLevel, nUpperLevel_16s, d_buffer), 
              NPP_NULL_POINTER_ERROR);
    
    // Test step error - too small step
    int invalidStep = width * sizeof(Npp16s) - 1;
    EXPECT_EQ(nppiHistogramEven_16s_C1R(d_src_16s, invalidStep, oSizeROI, d_hist, nLevels,
                                       nLowerLevel, nUpperLevel_16s, d_buffer), 
              NPP_STEP_ERROR);
}
...
TEST_F(HistogramEven16BitTest, SmallLevelsOptimization) {
    ...
    EXPECT_EQ(nppiHistogramEven_16u_C1R(d_src_16u, nSrcStep_16u, oSizeROI, d_hist, smallLevels,
                                       0, 65535, d_buffer), NPP_SUCCESS);
    
    ASSERT_EQ(cudaMemcpy(h_hist_small.data(), d_hist, (smallLevels - 1) * sizeof(Npp32s), 
                        cudaMemcpyDeviceToHost), cudaSuccess);
...
TEST_F(HistogramEven16BitTest, UniformDistribution) {
    ...
    int testUpper = 1024;
    EXPECT_EQ(nppiHistogramEven_16u_C1R(d_src_16u, nSrcStep_16u, oSizeROI, d_hist, nLevels,
                                       nLowerLevel, testUpper, d_buffer), NPP_SUCCESS);
