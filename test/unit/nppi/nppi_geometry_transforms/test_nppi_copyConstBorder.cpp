...
protected:
    ...
    template<typename T>
    void verifyCopyConstBorder_C1(const std::vector<T>& srcData, const std::vector<T>& dstData, 
                                 T borderValue) {
        for (int y = 0; y < dstHeight; y++) {
            for (int x = 0; x < dstWidth; x++) {
                ...
                // 检查是否在源区域内
                if (x >= nLeftBorderWidth && x < nLeftBorderWidth + srcWidth &&
                    y >= nTopBorderHeight && y < nTopBorderHeight + srcHeight) {
                    // 在源区域内，应该复制源数据
                    ...
                    // 在边界区域，应该是边界值
                    EXPECT_EQ(dstData[dstIdx], borderValue) 
                        << "Border value mismatch at (" << x << "," << y << ")";
                }
            }
        }
    }
...
TEST_F(NPPICopyConstBorderTest, CopyConstBorder_8u_C1R) {
    ...
    // 调用NPP函数
    NppStatus status = nppiCopyConstBorder_8u_C1R(d_src, nSrcStep, oSrcSizeROI,
...
TEST_F(NPPICopyConstBorderTest, CopyConstBorder_8u_C3R) {
    ...
    // 调用NPP函数
    NppStatus status = nppiCopyConstBorder_8u_C3R(d_src, nSrcStep, oSrcSizeROI,
                                                 d_dst, nDstStep, oDstSizeROI,
...
TEST_F(NPPICopyConstBorderTest, CopyConstBorder_16s_C1R) {
    ...
    // 调用NPP函数
    NppStatus status = nppiCopyConstBorder_16s_C1R(d_src, nSrcStep, oSrcSizeROI,
                                                  d_dst, nDstStep, oDstSizeROI,
                                                  nTopBorderHeight, nLeftBorderWidth, borderValue);
    EXPECT_EQ(status, NPP_SUCCESS);
...
TEST_F(NPPICopyConstBorderTest, CopyConstBorder_32f_C1R) {
    ...
    // 调用NPP函数
    NppStatus status = nppiCopyConstBorder_32f_C1R(d_src, nSrcStep, oSrcSizeROI,
                                                  d_dst, nDstStep, oDstSizeROI,
                                                  nTopBorderHeight, nLeftBorderWidth, borderValue);
    EXPECT_EQ(status, NPP_SUCCESS);
...
TEST_F(NPPICopyConstBorderTest, CopyConstBorder_8u_C1R_Ctx) {
    ...
    // 调用NPP函数
    NppStatus status = nppiCopyConstBorder_8u_C1R_Ctx(d_src, nSrcStep, oSrcSizeROI,
                                                      d_dst, nDstStep, oDstSizeROI,
