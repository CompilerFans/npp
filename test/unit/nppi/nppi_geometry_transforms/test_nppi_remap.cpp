...
TEST_F(RemapFunctionalTest, Remap_8u_C1R_Ctx_Identity) {
    ...
    // 执行重映射
    NppStatus status = nppiRemap_8u_C1R_Ctx(
        src.get(), srcSize, src.step(), srcROI,
        xMap.get(), xMap.step(), yMap.get(), yMap.step(),
        dst.get(), dst.step(), dstSizeROI,
...
TEST_F(RemapFunctionalTest, Remap_16u_C3R_Ctx_Mirror) {
    ...
    // 执行重映射
    NppStatus status = nppiRemap_16u_C3R_Ctx(
        src.get(), srcSize, src.step(), srcROI,
        xMap.get(), xMap.step(), yMap.get(), yMap.step(),
        dst.get(), dst.step(), dstSizeROI,
        NPPI_INTER_NN, nppStreamCtx);
...
TEST_F(RemapFunctionalTest, Remap_32f_C1R_Ctx_Rotation) {
    ...
    // 执行重映射
    NppStatus status = nppiRemap_32f_C1R_Ctx(
        src.get(), srcSize, src.step(), srcROI,
        xMap.get(), xMap.step(), yMap.get(), yMap.step(),
        dst.get(), dst.step(), dstSizeROI,
