...
// Test non-Ctx version for 8u C1R
TEST_F(RotateFunctionalTest, Rotate_8u_C1R_90Degrees) {
  const int width = 64, height = 64;
  ...
  NppStatus status = nppiRotate_8u_C1R(src.get(), srcSize, src.step(), srcROI, dst.get(), dst.step(), srcROI, angle,
                                       shiftX, shiftY, NPPI_INTER_NN);
  ...
  ASSERT_GT(nonZeroCount, 100);
}

// Test non-Ctx version for 8u C3R
TEST_F(RotateFunctionalTest, Rotate_8u_C3R_45Degrees) {
  const int width = 64, height = 64;
  ...
  NppStatus status = nppiRotate_8u_C3R(src.get(), srcSize, src.step(), srcROI, dst.get(), dst.step(), srcROI, angle,
                                       shiftX, shiftY, NPPI_INTER_LINEAR);
  ...
  ASSERT_GT(totalNonZero, 0);
}

// Test non-Ctx version for 32f C1R
TEST_F(RotateFunctionalTest, Rotate_32f_C1R_180Degrees) {
  const int width = 32, height = 32;
  ...
  ASSERT_GT(nonZeroCount, width * height / 4);
}
