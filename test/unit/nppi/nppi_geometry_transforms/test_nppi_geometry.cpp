...
TEST_F(GeometryTest, Mirror_HorizontalAxis) {
  ...
  // 执行水平轴镜像（上下翻转）
  NppStatus status = nppiMirror_8u_C1R(d_src, step_src, d_dst, step_dst, size, NPP_HORIZONTAL_AXIS);
...
TEST_F(GeometryTest, Mirror_VerticalAxis) {
  ...
  // 执行垂直轴镜像（左右翻转）
  NppStatus status = nppiMirror_8u_C1R(d_src, step_src, d_dst, step_dst, size, NPP_VERTICAL_AXIS);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(GeometryTest, Mirror_BothAxis) {
  ...
  // 执行双轴镜像（180度旋转）
  NppStatus status = nppiMirror_8u_C1R(d_src, step_src, d_dst, step_dst, size, NPP_BOTH_AXIS);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST_F(GeometryTest, Mirror_InPlace) {
  ...
  // 执行原地水平镜像
  NppStatus status = nppiMirror_8u_C1R(d_src, step_src, d_src, step_src, size, NPP_HORIZONTAL_AXIS);
  ASSERT_EQ(status, NPP_SUCCESS);
...
TEST(GeometryParameterTest, DISABLED_NullPointerError) {
  ...
  EXPECT_EQ(nppiMirror_8u_C1R(nullptr, step, &dummy, step, size, NPP_HORIZONTAL_AXIS), NPP_NULL_POINTER_ERROR);
...
TEST(GeometryParameterTest, DISABLED_InvalidSize) {
  ...
  // 测试无效宽度
  NppiSize size = {0, 10};
  EXPECT_EQ(nppiMirror_8u_C1R(&dummy, step, &dummy, step, size, NPP_HORIZONTAL_AXIS), NPP_SIZE_ERROR);
...
TEST(GeometryParameterTest, DISABLED_InvalidFlipAxis) {
  ...
  // 测试无效的flip参数
  EXPECT_EQ(nppiMirror_8u_C1R(&dummy, step, &dummy, step, size, (NppiAxis)999), NPP_MIRROR_FLIP_ERROR);
}
