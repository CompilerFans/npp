...
protected:
  ...
  template <typename T> T *allocateGpuMemory(size_t size) {
    T *ptr = nullptr;
  ...
  template <typename T> void copyToGpu(const std::vector<T> &src, T *dst) {
    cudaError_t err = cudaMemcpy(dst, src.data(), src.size() * sizeof(T), cudaMemcpyHostToDevice);
    EXPECT_EQ(err, cudaSuccess);
  }

  template <typename T> std::vector<T> copyFromGpu(const T *src, size_t size) {
  ...
  template <typename T> std::vector<T> createTestPattern(int width, int height, int channels) {
...
template <typename T>
void verifyHorizontalMirror(const std::vector<T> &original, const std::vector<T> &mirrored, int width, int height,
                            int channels) {
  for (int y = 0; y < height; ++y) {
    int srcY = height - 1 - y;
    for (int x = 0; x < width; ++x) {
...
template <typename T>
void verifyVerticalMirror(const std::vector<T> &original, const std::vector<T> &mirrored, int width, int height,
                          int channels) {
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      int srcX = width - 1 - x;
...
template <typename T>
void verifyBothAxisMirror(const std::vector<T> &original, const std::vector<T> &mirrored, int width, int height,
                          int channels) {
  for (int y = 0; y < height; ++y) {
    int srcY = height - 1 - y;
    for (int x = 0; x < width; ++x) {
...
TEST_F(NppiMirrorTest, 8u_C1R_Horizontal) {
  ...
  NppStatus status =
      nppiMirror_8u_C1R(d_src, width * sizeof(Npp8u), d_dst, width * sizeof(Npp8u), roi, NPP_HORIZONTAL_AXIS);
...
TEST_F(NppiMirrorTest, 8u_C1R_Vertical) {
  ...
  NppiSize roi = {width, height};
  NppStatus status =
      nppiMirror_8u_C1R(d_src, width * sizeof(Npp8u), d_dst, width * sizeof(Npp8u), roi, NPP_VERTICAL_AXIS);
...
TEST_F(NppiMirrorTest, 8u_C1R_Both) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_8u_C1R(d_src, width * sizeof(Npp8u), d_dst, width * sizeof(Npp8u), roi, NPP_BOTH_AXIS);
...
TEST_F(NppiMirrorTest, 8u_C3R_Horizontal) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_8u_C3R(d_src, width * channels * sizeof(Npp8u), d_dst, width * channels * sizeof(Npp8u),
...
TEST_F(NppiMirrorTest, 8u_C4R_Horizontal) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_8u_C4R(d_src, width * channels * sizeof(Npp8u), d_dst, width * channels * sizeof(Npp8u),
                                       roi, NPP_HORIZONTAL_AXIS);
...
TEST_F(NppiMirrorTest, 16u_C1R_Horizontal) {
  ...
  NppiSize roi = {width, height};
  NppStatus status =
      nppiMirror_16u_C1R(d_src, width * sizeof(Npp16u), d_dst, width * sizeof(Npp16u), roi, NPP_HORIZONTAL_AXIS);
...
TEST_F(NppiMirrorTest, 32f_C1R_Horizontal) {
  ...
  NppiSize roi = {width, height};
  NppStatus status =
      nppiMirror_32f_C1R(d_src, width * sizeof(Npp32f), d_dst, width * sizeof(Npp32f), roi, NPP_HORIZONTAL_AXIS);
...
TEST_F(NppiMirrorTest, 32s_C1R_Horizontal) {
  ...
  NppiSize roi = {width, height};
  NppStatus status =
      nppiMirror_32s_C1R(d_src, width * sizeof(Npp32s), d_dst, width * sizeof(Npp32s), roi, NPP_HORIZONTAL_AXIS);
...
TEST_F(NppiMirrorTest, 8u_C3IR_Vertical) {
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      int srcX = width - 1 - x;
      for (int c = 0; c < channels; ++c) {
        expectedData[(y * width + x) * channels + c] = srcData[(y * width + srcX) * channels + c];
      }
    }
  }
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_8u_C3IR(d_src, width * channels * sizeof(Npp8u), roi, NPP_VERTICAL_AXIS);
...
TEST_F(NppiMirrorTest, 8u_C1R_Ctx) {
  ...
  NppStatus status =
      nppiMirror_8u_C1R_Ctx(d_src, width * sizeof(Npp8u), d_dst, width * sizeof(Npp8u), roi, NPP_HORIZONTAL_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 16u_C3R_Vertical) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_16u_C3R(d_src, width * channels * sizeof(Npp16u), d_dst,
                                        width * channels * sizeof(Npp16u), roi, NPP_VERTICAL_AXIS);
...
TEST_F(NppiMirrorTest, 16u_C4R_Both) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_16u_C4R(d_src, width * channels * sizeof(Npp16u), d_dst,
                                        width * channels * sizeof(Npp16u), roi, NPP_BOTH_AXIS);
...
TEST_F(NppiMirrorTest, 16u_C3IR_Horizontal) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      for (int c = 0; c < channels; ++c) {
        expectedData[(y * width + x) * channels + c] = srcData[(srcY * width + x) * channels + c];
      }
    }
  }
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_16u_C3IR(d_src, width * channels * sizeof(Npp16u), roi, NPP_HORIZONTAL_AXIS);
...
TEST_F(NppiMirrorTest, 16u_C4IR_Vertical) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_16u_C4IR(d_src, width * channels * sizeof(Npp16u), roi, NPP_VERTICAL_AXIS);
...
TEST_F(NppiMirrorTest, 16u_C1R_Ctx) {
  ...
  NppStatus status =
      nppiMirror_16u_C1R_Ctx(d_src, width * sizeof(Npp16u), d_dst, width * sizeof(Npp16u), roi, NPP_VERTICAL_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 32f_C3R_Vertical) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32f_C3R(d_src, width * channels * sizeof(Npp32f), d_dst,
                                        width * channels * sizeof(Npp32f), roi, NPP_VERTICAL_AXIS);
...
TEST_F(NppiMirrorTest, 32f_C4R_Both) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32f_C4R(d_src, width * channels * sizeof(Npp32f), d_dst,
                                        width * channels * sizeof(Npp32f), roi, NPP_BOTH_AXIS);
...
TEST_F(NppiMirrorTest, 32f_C3IR_Both) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      int srcX = width - 1 - x;
      for (int c = 0; c < channels; ++c) {
        expectedData[(y * width + x) * channels + c] = srcData[(srcY * width + srcX) * channels + c];
      }
    }
  }
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32f_C3IR(d_src, width * channels * sizeof(Npp32f), roi, NPP_BOTH_AXIS);
...
TEST_F(NppiMirrorTest, 32f_C4IR_Horizontal) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32f_C4IR(d_src, width * channels * sizeof(Npp32f), roi, NPP_HORIZONTAL_AXIS);
...
TEST_F(NppiMirrorTest, 32f_C3R_Ctx) {
  ...
  NppStatus status = nppiMirror_32f_C3R_Ctx(d_src, width * channels * sizeof(Npp32f), d_dst,
                                            width * channels * sizeof(Npp32f), roi, NPP_HORIZONTAL_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 32s_C3R_Vertical) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32s_C3R(d_src, width * channels * sizeof(Npp32s), d_dst,
                                        width * channels * sizeof(Npp32s), roi, NPP_VERTICAL_AXIS);
...
TEST_F(NppiMirrorTest, 32s_C4R_Both) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32s_C4R(d_src, width * channels * sizeof(Npp32s), d_dst,
                                        width * channels * sizeof(Npp32s), roi, NPP_BOTH_AXIS);
...
TEST_F(NppiMirrorTest, 32s_C3IR_Vertical) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32s_C3IR(d_src, width * channels * sizeof(Npp32s), roi, NPP_VERTICAL_AXIS);
...
TEST_F(NppiMirrorTest, 32s_C4IR_Both) {
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_32s_C4IR(d_src, width * channels * sizeof(Npp32s), roi, NPP_BOTH_AXIS);
...
TEST_F(NppiMirrorTest, 32s_C4R_Ctx) {
  ...
  NppStatus status = nppiMirror_32s_C4R_Ctx(d_src, width * channels * sizeof(Npp32s), d_dst,
                                            width * channels * sizeof(Npp32s), roi, NPP_BOTH_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 8u_C3R_Ctx) {
  ...
  NppStatus status = nppiMirror_8u_C3R_Ctx(d_src, width * channels * sizeof(Npp8u), d_dst,
                                           width * channels * sizeof(Npp8u), roi, NPP_VERTICAL_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 8u_C4R_Ctx) {
  ...
  NppStatus status = nppiMirror_8u_C4R_Ctx(d_src, width * channels * sizeof(Npp8u), d_dst,
                                           width * channels * sizeof(Npp8u), roi, NPP_BOTH_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 8u_C1IR_Ctx) {
  ...
  NppStatus status = nppiMirror_8u_C1IR_Ctx(d_src, width * sizeof(Npp8u), roi, NPP_HORIZONTAL_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 8u_C3IR_Ctx) {
  ...
  NppStatus status = nppiMirror_8u_C3IR_Ctx(d_src, width * channels * sizeof(Npp8u), roi, NPP_VERTICAL_AXIS, ctx);
...
TEST_F(NppiMirrorTest, 8u_C4IR_Ctx) {
  ...
  NppStatus status = nppiMirror_8u_C4IR_Ctx(d_src, width * channels * sizeof(Npp8u), roi, NPP_BOTH_AXIS, ctx);
...
TEST_F(NppiMirrorTest, WithPaddedStep) {
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      for (int c = 0; c < channels; ++c) {
        srcPadded[y * paddedStep + x * channels + c] = srcData[(y * width + x) * channels + c];
      }
    }
  }
  ...
  NppiSize roi = {width, height};
  NppStatus status = nppiMirror_8u_C3R(d_src, paddedStep, d_dst, paddedStep, roi, NPP_HORIZONTAL_AXIS);
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      for (int c = 0; c < channels; ++c) {
        result[(y * width + x) * channels + c] = resultPadded[y * paddedStep + x * channels + c];
      }
    }
  }
