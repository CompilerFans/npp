...
TEST_F(TransposeFunctionalTest, DISABLED_ErrorHandling_NullPointer) {
  ...
  // 测试null源指针
  NppStatus status = nppiTranspose_32f_C1R(nullptr, src.step(), dst.get(), dst.step(), roi);

  EXPECT_EQ(status, NPP_NULL_POINTER_ERROR) << "Should detect null source pointer";

  // 测试null目标指针
  status = nppiTranspose_32f_C1R(src.get(), src.step(), nullptr, dst.step(), roi);
...
TEST_F(TransposeFunctionalTest, DISABLED_ErrorHandling_InvalidROI) {
  ...
  NppStatus status = nppiTranspose_32f_C1R(src.get(), src.step(), dst.get(), dst.step(), invalid_roi);

  EXPECT_EQ(status, NPP_SIZE_ERROR) << "Should detect invalid ROI width";

  // 测试负数ROI
  invalid_roi = {size, -1}; // 负高度
  status = nppiTranspose_32f_C1R(src.get(), src.step(), dst.get(), dst.step(), invalid_roi);
...
TEST_F(TransposeFunctionalTest, Performance_VariousSizes) {
  ...
  std::vector<TestSize> test_sizes = {{32, 32, "Small 32x32"},
