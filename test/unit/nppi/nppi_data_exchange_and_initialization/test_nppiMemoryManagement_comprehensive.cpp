...
TEST_F(NppiMemoryManagementTest, nppiMalloc_32f_C1_VariousDimensions) {
    ...
    for (const auto& dim : dimensions) {
        ...
        if (ptr != nullptr) {
            EXPECT_GE(step, dim.width * sizeof(Npp32f)) 
                << "Insufficient step for " << dim.description;
            EXPECT_TRUE(isAligned(ptr, 256)) 
                << "Bad alignment for " << dim.description;
...
TEST_F(NppiMemoryManagementTest, nppiMalloc_32f_C3_VariousDimensions) {
    ...
    for (const auto& dim : dimensions) {
        ...
        if (ptr != nullptr) {
            EXPECT_GE(step, dim.width * 3 * sizeof(Npp32f)) 
                << "Insufficient step for " << dim.description;
            EXPECT_TRUE(isAligned(ptr, 256)) 
                << "Bad alignment for " << dim.description;
            EXPECT_EQ(step % 256, 0)
                << "Step not aligned for " << dim.description;
...
TEST_F(NppiMemoryManagementTest, nppiMalloc_nppiFree_MemoryReusePatterns) {
    ...
    EXPECT_LT(allocatedAddresses.size(), iterations) 
        << "Memory doesn't seem to be reused efficiently";
...
TEST_F(NppiMemoryManagementTest, nppiMalloc_32f_ConcurrentAllocations) {
    ...
    EXPECT_EQ(uniquePtrs.size(), numAllocations * 2) 
        << "Some pointers are not unique";
...
TEST_F(NppiMemoryManagementTest, nppiMalloc_32f_AlignmentVerification) {
    ...
    for (int width : testCases) {
        // Test C1
        {
            ...
            if (ptr != nullptr) {
                EXPECT_TRUE(isAligned(ptr, 256)) 
                    << "C1 pointer not aligned for width " << width;
                EXPECT_EQ(step % 256, 0) 
                    << "C1 step not aligned for width " << width;
                
                // Calculate minimum required step
                int minStep = width * sizeof(Npp32f);
                int alignedStep = ((minStep + 255) / 256) * 256;
                EXPECT_EQ(step, alignedStep) 
                    << "C1 step calculation wrong for width " << width;
        ...
        // Test C3
        {
            ...
            if (ptr != nullptr) {
                EXPECT_TRUE(isAligned(ptr, 256)) 
                    << "C3 pointer not aligned for width " << width;
                EXPECT_EQ(step % 256, 0) 
                    << "C3 step not aligned for width " << width;
                
                // Calculate minimum required step
                int minStep = width * 3 * sizeof(Npp32f);
                int alignedStep = ((minStep + 255) / 256) * 256;
                EXPECT_EQ(step, alignedStep) 
                    << "C3 step calculation wrong for width " << width;
...
TEST_F(NppiMemoryManagementTest, nppiFree_NullPointer) {
...
TEST_F(NppiMemoryManagementTest, nppiMalloc_32f_MemoryPatternPreservation) {
    ...
    // Test C1
    {
        ...
        cudaMemcpy2D(ptr, step, pattern.data(), width * sizeof(Npp32f),
                     width * sizeof(Npp32f), height, cudaMemcpyHostToDevice);
        
        // Read back and verify
        std::vector<Npp32f> readback(width * height);
        cudaMemcpy2D(readback.data(), width * sizeof(Npp32f), ptr, step,
    ...
    // Test C3
    {
        ...
        cudaMemcpy2D(ptr, step, pattern.data(), width * 3 * sizeof(Npp32f),
                     width * 3 * sizeof(Npp32f), height, cudaMemcpyHostToDevice);
        
        // Read back and verify
        std::vector<Npp32f> readback(width * height * 3);
        cudaMemcpy2D(readback.data(), width * 3 * sizeof(Npp32f), ptr, step,
