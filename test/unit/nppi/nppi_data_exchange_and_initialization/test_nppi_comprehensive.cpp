...
class NPPIComprehensiveTest : public NppTestBase {
protected:
    void SetUp() override {
        NppTestBase::SetUp();
    }
...
TEST_F(NPPIComprehensiveTest, Convert_8u32f_C3R_VariousSizes) {
    struct TestSize {
        int width, height;
    ...
    TestSize testSizes[] = {
        ...
        {3, 1, "Single row"},
        {1, 3, "Single column"},
        {15, 15, "Small odd size"},
        {16, 16, "Small power of 2"},
        {17, 17, "Small prime"},
        {256, 256, "Medium size"},
    ...
    for (const auto& testSize : testSizes) {
        ...
        NppStatus status = nppiConvert_8u32f_C3R(src.get(), src.step(), 
                                                 dst.get(), dst.step(), oSizeROI);

        ASSERT_EQ(status, NPP_SUCCESS) << "Failed for " << testSize.description 
                                       << " (" << testSize.width << "x" << testSize.height << ")";
...
TEST_F(NPPIComprehensiveTest, Convert_8u32f_C3R_NonStandardSteps) {
    ...
    cudaMemcpy2D(dstData.data(), width * channels * sizeof(Npp32f),
                 d_dst, dstStep,
                 width * channels * sizeof(Npp32f), height,
...
TEST_F(NPPIComprehensiveTest, Convert_8u32f_C3R_ConcurrentStreams) {
    ...
    for (int i = 0; i < numStreams; i++) {
        NppStatus status = nppiConvert_8u32f_C3R_Ctx(
            srcs[i].get(), srcs[i].step(),
            dsts[i].get(), dsts[i].step(),
            oSizeROI, contexts[i]
        );
    ...
    for (int i = 1; i < numStreams; i++) {
        ...
        for (size_t j = 0; j < result.size(); j++) {
            EXPECT_FLOAT_EQ(result[j], baseResult[j]) 
                << "Stream " << i << " differs at index " << j;
        }
    }
...
TEST_F(NPPIComprehensiveTest, Copy_32f_C1R_OverlappingRegions) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            EXPECT_FLOAT_EQ(result[dstIdx], srcData[srcIdx])
                << "Mismatch at destination (" << x << "," << y + overlap << ")";
        }
    }
}

// Test with various step configurations
TEST_F(NPPIComprehensiveTest, Copy_32f_C1R_DifferentSteps) {
    ...
    for (const auto& config : configs) {
        ...
        cudaMemcpy(d_src, srcData.data(), srcData.size() * sizeof(Npp32f), 
        ...
        cudaMemcpy(dstData.data(), d_dst, dstData.size() * sizeof(Npp32f),
                   cudaMemcpyDeviceToHost);
...
TEST_F(NPPIComprehensiveTest, Copy_32f_C1R_Performance) {
    ...
    for (int i = 0; i < iterations; i++) {
        NppStatus status = nppiCopy_32f_C1R(src.get(), src.step(), 
    ...
    std::cout << "Copy_32f_C1R Performance: " << avgTime << " us, "
...
TEST_F(NPPIComprehensiveTest, Copy_32f_C3P3R_ROIVariations) {
    ...
    ROITest roiTests[] = {
        {0, 0, fullWidth, fullHeight, "Full image"},
    ...
    NppImageMemory<Npp32f> dstPlanes[3] = {
    ...
    for (const auto& roiTest : roiTests) {
        ...
        Npp32f* dstPtrs[3] = {
            dstPlanes[0].get() + roiTest.y * (dstPlanes[0].step()/sizeof(Npp32f)) + roiTest.x,
        ...
        NppStatus status = nppiCopy_32f_C3P3R(pSrc, src.step(), dstPtrs, 
                                             dstPlanes[0].step(), oSizeROI);
...
TEST_F(NPPIComprehensiveTest, Copy_32f_C3P3R_UnalignedMemory) {
    ...
    cudaMemcpy(src, srcData.data(), srcData.size() * sizeof(Npp32f), 
               cudaMemcpyHostToDevice);
    
    // Copy with unaligned pointers
    NppiSize oSizeROI = {width, height};
    NppStatus status = nppiCopy_32f_C3P3R(src, width * 3 * sizeof(Npp32f),
                                         dstPtrs, width * sizeof(Npp32f),
                                         oSizeROI);
    ...
    for (int p = 0; p < 3; p++) {
        ...
        cudaMemcpy(planeData.data(), dstPtrs[p], planeData.size() * sizeof(Npp32f),
                   cudaMemcpyDeviceToHost);
        
        for (int i = 0; i < width * height; i++) {
            int srcIdx = i * 3 + p;
            EXPECT_FLOAT_EQ(planeData[i], srcData[srcIdx])
                << "Mismatch in plane " << p << " at index " << i;
        }
    }
...
TEST_F(NPPIComprehensiveTest, Copy_32f_C3R_InPlace) {
    const int width = 128, height = 128;
    ...
    NppiSize roi = {width, height};
    NppStatus status = nppiCopy_32f_C3R(buffer.get(), buffer.step(),
                                       buffer.get(), buffer.step(), roi);
    ...
    for (size_t i = 0; i < resultData.size(); i++) {
        EXPECT_FLOAT_EQ(resultData[i], originalData[i])
            << "Data changed at index " << i;
    }
}

// Test with maximum supported dimensions
TEST_F(NPPIComprehensiveTest, Copy_32f_C3R_MaxDimensions) {
    // Test various extreme dimensions
    struct DimTest {
        int width, height;
        const char* description;
    };
    ...
    DimTest dimTests[] = {
        ...
        {1, 1, "Single pixel"},
        {3, 1, "Single pixel 3 channel"}
    };
    
    for (const auto& dim : dimTests) {
        // Skip if too large for available memory
        size_t requiredMem = dim.width * dim.height * 3 * sizeof(Npp32f) * 2;
        size_t freeMem, totalMem;
        cudaMemGetInfo(&freeMem, &totalMem);
        
        if (requiredMem > freeMem * 0.8) {
            std::cout << "Skipping " << dim.description << " - insufficient memory" << std::endl;
            continue;
        }
        
        NppImageMemory<Npp32f> src(dim.width * 3, dim.height);
        NppImageMemory<Npp32f> dst(dim.width * 3, dim.height);
        
        // Initialize with pattern
        std::vector<Npp32f> srcData(dim.width * dim.height * 3);
        for (size_t i = 0; i < srcData.size(); i++) {
            srcData[i] = static_cast<float>(i % 1000);
        }
        src.copyFromHost(srcData);
        
        NppiSize roi = {dim.width, dim.height};
        NppStatus status = nppiCopy_32f_C3R(src.get(), src.step(),
                                           dst.get(), dst.step(), roi);
        
        ASSERT_EQ(status, NPP_SUCCESS) << "Failed for " << dim.description;
        
        // Sample verification
        std::vector<Npp32f> sample(std::min(1000, dim.width * dim.height * 3));
        cudaMemcpy(sample.data(), dst.get(), sample.size() * sizeof(Npp32f),
                   cudaMemcpyDeviceToHost);
        
        for (size_t i = 0; i < sample.size(); i++) {
            EXPECT_FLOAT_EQ(sample[i], srcData[i])
                << "Mismatch at sample " << i << " for " << dim.description;
        }
    }
}

// Concurrent copy operations
TEST_F(NPPIComprehensiveTest, Copy_32f_C3R_ConcurrentOperations) {
    const int width = 512, height = 512;
    ...
    for (int i = 0; i < numOperations; i++) {
        NppStatus status = nppiCopy_32f_C3R_Ctx(
            sources[i]->get(), sources[i]->step(),
            destinations[i]->get(), destinations[i]->step(),
            roi, contexts[i]
        );
    ...
    std::cout << "Concurrent copy operations completed in " 
    ...
    for (int i = 0; i < numOperations; i++) {
        ...
        for (int j = 0; j < 1000; j += 100) {
            EXPECT_FLOAT_EQ(result[j], expectedResult[j])
                << "Stream " << i << " result mismatch at " << j;
        }
    }
...
TEST_F(NPPIComprehensiveTest, MemoryManagement_AlignmentTest) {
    ...
    for (int i = 0; i < numAllocations; i++) {
        ...
        ASSERT_EQ(reinterpret_cast<uintptr_t>(ptr1) % 256, 0) 
            << "Pointer " << i << " not 256-byte aligned";
        ASSERT_GE(step, width * sizeof(Npp32f))
            << "Step too small for allocation " << i;
        ASSERT_EQ(step % 256, 0)
            << "Step not 256-byte aligned for allocation " << i;
        ...
        ASSERT_EQ(reinterpret_cast<uintptr_t>(ptr3) % 256, 0)
            << "C3 pointer " << i << " not 256-byte aligned";
        ASSERT_GE(step, width * 3 * sizeof(Npp32f))
            << "C3 step too small for allocation " << i;
...
TEST_F(NPPIComprehensiveTest, MemoryManagement_StressTest) {
    ...
    for (int cycle = 0; cycle < cycles; cycle++) {
        ...
        if (allocations.size() < maxAllocations && actionDist(gen) == 0) {
            ...
            Npp32f* ptr = (cycle % 2 == 0) 
                ? nppiMalloc_32f_C1(width, height, &step)
                : nppiMalloc_32f_C3(width, height, &step);
            ...
            cudaMemcpy(check.data(), allocations[idx], 
                      check.size() * sizeof(Npp32f),
                      cudaMemcpyDeviceToHost);
...
TEST_F(NPPIComprehensiveTest, MemoryManagement_LimitsAndErrors) {
    ...
    nppiFree(ptr);
    nppiFree(ptr); // Should handle gracefully
}
...
TEST_F(NPPIComprehensiveTest, MemoryManagement_UsagePatterns) {
    ...
    std::vector<std::pair<Npp32f*, size_t>> allocations;
    size_t totalAllocated = 0;
    
    for (int size = 16; size <= 1024; size *= 2) {
        int step;
        Npp32f* ptr = nppiMalloc_32f_C1(size, size, &step);
        if (ptr) {
            size_t allocSize = step * size;
    ...
    size_t actualUsed = freeStart - freeAfter;
    EXPECT_GE(actualUsed, totalAllocated) 
        << "Actual memory used less than allocated";
    
    // The overhead should be reasonable (< 2x)
    EXPECT_LT(actualUsed, totalAllocated * 2)
        << "Memory overhead too large";
    
    // Free all
    for (auto& alloc : allocations) {
        nppiFree(alloc.first);
    }
    
    // Verify memory is reclaimed
    size_t freeEnd, totalEnd;
    cudaMemGetInfo(&freeEnd, &totalEnd);
    EXPECT_NEAR(freeEnd, freeStart, freeStart * 0.01)
        << "Memory not fully reclaimed";
}
...
TEST_F(NPPIComprehensiveTest, ErrorHandling_InvalidParameters) {
    int width = 100, height = 100;
    int step;
    
    // Valid allocations for testing
    Npp8u* src8u = nppiMalloc_8u_C3(width, height, &step);
    Npp32f* dst32f = nppiMalloc_32f_C3(width, height, &step);
    Npp32f* planes[3];
    for (int i = 0; i < 3; i++) {
        planes[i] = nppiMalloc_32f_C1(width, height, &step);
    }
    
    NppiSize validROI = {width, height};
    NppiSize invalidROI = {0, 0};
    
    // Test nppiConvert_8u32f_C3R errors
    EXPECT_EQ(nppiConvert_8u32f_C3R(nullptr, step, dst32f, step, validROI), 
              NPP_NULL_POINTER_ERROR);
    EXPECT_EQ(nppiConvert_8u32f_C3R(src8u, step, nullptr, step, validROI),
              NPP_NULL_POINTER_ERROR);
    EXPECT_EQ(nppiConvert_8u32f_C3R(src8u, 0, dst32f, step, validROI),
              NPP_STEP_ERROR);
    EXPECT_EQ(nppiConvert_8u32f_C3R(src8u, step, dst32f, step, invalidROI),
              NPP_SIZE_ERROR);
    
    // Test nppiCopy errors
    EXPECT_EQ(nppiCopy_32f_C3R(nullptr, step, dst32f, step, validROI),
              NPP_NULL_POINTER_ERROR);
    EXPECT_EQ(nppiCopy_32f_C3P3R(dst32f, step, nullptr, step, validROI),
              NPP_NULL_POINTER_ERROR);
