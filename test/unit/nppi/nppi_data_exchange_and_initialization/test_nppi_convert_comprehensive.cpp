...
TEST_F(NPPIConvertAndCopyComprehensiveTest, nppiMalloc_32f_UsagePatterns) {
  // Track memory usage
  ...
  EXPECT_GE(actualUsed, totalAllocated) << "Actual memory used less than allocated";

  // The overhead should be reasonable (< 2x)
  EXPECT_LT(actualUsed, totalAllocated * 2) << "Memory overhead too large";
  ...
  EXPECT_NEAR(freeEnd, freeStart, freeStart * 0.01) << "Memory not fully reclaimed";
}
