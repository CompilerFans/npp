...
TEST_F(FilterConvolutionFunctionalTest, DISABLED_Filter_ErrorHandling) {
  ...
  // 测试空指针
  NppStatus status =
      nppiFilter_8u_C1R(nullptr, src.step(), dst.get(), dst.step(), roi, kernel.data(), kernelSize, anchor, 1);
  EXPECT_NE(status, NPP_SUCCESS) << "Should fail with null source pointer";

  // 测试无效ROI
  NppiSize invalidRoi = {0, 0};
  status =
      nppiFilter_8u_C1R(src.get(), src.step(), dst.get(), dst.step(), invalidRoi, kernel.data(), kernelSize, anchor, 1);
  ...
  // 测试零除数
  status = nppiFilter_8u_C1R(src.get(), src.step(), dst.get(), dst.step(), roi, kernel.data(), kernelSize, anchor, 0);
...
TEST_F(FilterConvolutionFunctionalTest, DISABLED_Filter_DifferentKernelSizes) {
  ...
  NppStatus status3x3 = nppiFilter_8u_C1R(src.get(), src.step(), dst3x3.get(), dst3x3.step(), roi, kernel3x3.data(),
                                          kernelSize3x3, anchor3x3, 9);
  ...
  NppStatus status5x5 = nppiFilter_8u_C1R(src.get(), src.step(), dst5x5.get(), dst5x5.step(), roi, kernel5x5.data(),
                                          kernelSize5x5, anchor5x5, 25);
