...
protected:
    ...
    float calculateGaussianValue(const std::vector<Npp8u>& src, int width, int height, 
                                int x, int y, int kernelSize, float sigma) {
        float result = 0.0f;
...
TEST_F(GaussianFilterFunctionalTest, FilterGauss_32f_C1R_Float) {
    ...
    NppiSize roi = {width, height};
    NppStatus status = nppiFilterGauss_32f_C1R(
        src.get(), src.step(),
        dst.get(), dst.step(),
        roi, NPP_MASK_SIZE_3_X_3);
...
TEST_F(GaussianFilterFunctionalTest, FilterGauss_ErrorHandling) {
    ...
    status = nppiFilterGauss_8u_C1R(
        src.get(), src.step(),
        dst.get(), dst.step(),
        roi, NPP_MASK_SIZE_1_X_3); // 不支持的大小
...
TEST_F(GaussianFilterFunctionalTest, FilterGauss_DifferentKernelSizes) {
    ...
    // 测试不同核大小
    NppStatus status3x3 = nppiFilterGauss_8u_C1R(
        src.get(), src.step(), dst3x3.get(), dst3x3.step(), roi, NPP_MASK_SIZE_3_X_3);
    NppStatus status5x5 = nppiFilterGauss_8u_C1R(
        src.get(), src.step(), dst5x5.get(), dst5x5.step(), roi, NPP_MASK_SIZE_5_X_5);
    NppStatus status7x7 = nppiFilterGauss_8u_C1R(
        src.get(), src.step(), dst7x7.get(), dst7x7.step(), roi, NPP_MASK_SIZE_7_X_7);
