...
public:
  ...
  static float calculatePositionWeight(int x, int y, int imgW, int imgH, int maskW, int maskH, int anchorX,
                                       int anchorY) {
    ...
    if (x == 0 || x == imgW - 1)
      xCorrection *= 0.89f; // Corner correction
    if (y == 0 || y == imgH - 1)
      yCorrection *= 0.89f; // Corner correction

    return baseWeight * xCorrection * yCorrection;
  }

  // Predict NPP FilterBox output for a single pixel
  static float predictPixelValue(const std::vector<uint8_t> &input, int x, int y, int imgW, int imgH, int maskW,
  ...
  static std::vector<uint8_t> predictFullOutput(const std::vector<uint8_t> &input, int imgW, int imgH, int maskW,
                                                int maskH, int anchorX, int anchorY) {
...
public:
  ...
  static std::vector<uint8_t> generateImpulsePattern(int width, int height, int impulseX, int impulseY,
                                                     uint8_t impulseValue) {
  ...
  static std::vector<uint8_t> generateBinaryStepPattern(int width, int height, uint8_t leftValue, uint8_t rightValue) {
  ...
  static std::vector<uint8_t> generateCheckerboardPattern(int width, int height, uint8_t value1, uint8_t value2) {
    std::vector<uint8_t> pattern(width * height);
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
...
public:
  ...
  static std::vector<uint8_t> applyBorderReplication(const std::vector<uint8_t> &input, int imgW, int imgH, int maskW,
                                                     int maskH, int anchorX, int anchorY) {
...
public:
  ...
  static bool verifyPrediction(const std::vector<uint8_t> &predicted, const std::vector<uint8_t> &actual,
                               int tolerance = 2) {
  ...
  static void calculateDifferenceStats(const std::vector<uint8_t> &result1, const std::vector<uint8_t> &result2,
    ...
    for (int d = 0; d <= maxDiff && d < 20; d++) {
      if (diffCounts[d] > 0) {
        ...
        std::cout << "  Diff " << std::setw(2) << d << ": " << std::setw(4) << diffCounts[d] << " pixels ("
  ...
  static void printComparison(const std::vector<uint8_t> &predicted, const std::vector<uint8_t> &actual, int width,
                              int height) {
...
protected:
  ...
  std::vector<uint8_t> runNPPFilterBox(const std::vector<uint8_t> &input, int width, int height, int maskW, int maskH,
...
TEST_F(FilterBoxAlgorithmTest, BorderReplicationVsNPPComparison) {
  ...
  std::vector<TestConfig> configs = {{4, 4, 3, 3, 1, 1, "4x4_3x3_center"},
  ...
  for (const auto &config : configs) {
    ...
    // Test uniform input
    auto uniformInput = AlgorithmTestGenerator::generateUniformPattern(config.width, config.height, 128);

    auto nppResult = runNPPFilterBox(uniformInput, config.width, config.height, config.maskW, config.maskH,
                                     config.anchorX, config.anchorY);
    auto borderRepResult = BorderReplicationAlgorithm::applyBorderReplication(
        uniformInput, config.width, config.height, config.maskW, config.maskH, config.anchorX, config.anchorY);
    ...
    auto stepInput = AlgorithmTestGenerator::generateBinaryStepPattern(config.width, config.height, 0, 255);

    nppResult = runNPPFilterBox(stepInput, config.width, config.height, config.maskW, config.maskH, config.anchorX,
                                config.anchorY);
    borderRepResult = BorderReplicationAlgorithm::applyBorderReplication(
        stepInput, config.width, config.height, config.maskW, config.maskH, config.anchorX, config.anchorY);

    std::cout << "\nBinary step input (0|255):\n";
    AlgorithmVerification::calculateDifferenceStats(nppResult, borderRepResult, "NPP", "BorderReplication");

    // Test impulse input
    auto impulseInput = AlgorithmTestGenerator::generateImpulsePattern(config.width, config.height, 0, 0, 255);

    nppResult = runNPPFilterBox(impulseInput, config.width, config.height, config.maskW, config.maskH, config.anchorX,
                                config.anchorY);
    borderRepResult = BorderReplicationAlgorithm::applyBorderReplication(
        impulseInput, config.width, config.height, config.maskW, config.maskH, config.anchorX, config.anchorY);
...
TEST_F(FilterBoxAlgorithmTest, ComprehensiveBorderReplicationAnalysis) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      if ((x + y) % 3 == 0)
        value = 255 - value; // Add some noise
  ...
  auto borderRepResult =
      BorderReplicationAlgorithm::applyBorderReplication(complexInput, width, height, maskW, maskH, anchorX, anchorY);
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      bool isBoundary = (x < anchorX) || (x >= width - (maskW - anchorX - 1)) || (y < anchorY) ||
                        (y >= height - (maskH - anchorY - 1));
