...
protected:
  ...
  template <typename T> void referenceBoxFilter(const std::vector<T> &srcData, std::vector<T> &refResult) {
  ...
  void verifyFilterResult(const std::vector<T> &result, const std::vector<T> &reference, T tolerance = T(1)) {
    ...
    for (size_t i = 0; i < result.size(); i++) {
      ...
      EXPECT_LE(diff, tolerance) << "Mismatch at index " << i << " result=" << (int)result[i]
  ...
  void testBorderMode(NppiBorderType borderType, const char *modeName, bool expectSupported = true) {
    ...
    // 按行复制数据到GPU
    for (int y = 0; y < srcHeight; y++) {
      cudaMemcpy((char *)d_src + y * nSrcStep, srcData.data() + y * srcWidth, srcWidth * sizeof(Npp8u),
                 cudaMemcpyHostToDevice);
    }

    // 测试边界模式
    NppStatus status = nppiFilterBoxBorder_8u_C1R(d_src, nSrcStep, oSrcSizeROI, oSrcOffset, d_dst, nDstStep,
                                                  oDstSizeROI, oMaskSize, oAnchor, borderType);
    ...
    if (expectSupported) {
      ...
      if (status == NPP_SUCCESS) {
        ...
        for (int y = 0; y < dstHeight; y++) {
          cudaMemcpy(dstData.data() + y * dstWidth, (char *)d_dst + y * nDstStep, dstWidth * sizeof(Npp8u),
...
TEST_F(NPPIFilterBoxBorderTest, FilterBoxBorder_8u_C3R) {
  ...
  // 按行复制数据到GPU
  for (int y = 0; y < srcHeight; y++) {
    cudaMemcpy((char *)d_src + y * nSrcStep, srcData.data() + y * srcWidth * 3, srcWidth * 3 * sizeof(Npp8u),
               cudaMemcpyHostToDevice);
  }
  ...
  // 按行复制结果回主机
  for (int y = 0; y < dstHeight; y++) {
    cudaMemcpy(dstData.data() + y * dstWidth * 3, (char *)d_dst + y * nDstStep, dstWidth * 3 * sizeof(Npp8u),
               cudaMemcpyDeviceToHost);
  }
...
TEST_F(NPPIFilterBoxBorderTest, FilterBoxBorder_16s_C1R) {
  ...
  // 按行复制数据到GPU
  for (int y = 0; y < srcHeight; y++) {
    cudaMemcpy((char *)d_src + y * nSrcStep, srcData.data() + y * srcWidth, srcWidth * sizeof(Npp16s),
               cudaMemcpyHostToDevice);
  }
  ...
  // 按行复制结果回主机
  for (int y = 0; y < dstHeight; y++) {
    cudaMemcpy(dstData.data() + y * dstWidth, (char *)d_dst + y * nDstStep, dstWidth * sizeof(Npp16s),
               cudaMemcpyDeviceToHost);
  }
...
TEST_F(NPPIFilterBoxBorderTest, FilterBoxBorder_32f_C1R) {
  ...
  // 按行复制数据到GPU
  for (int y = 0; y < srcHeight; y++) {
    cudaMemcpy((char *)d_src + y * nSrcStep, srcData.data() + y * srcWidth, srcWidth * sizeof(Npp32f),
               cudaMemcpyHostToDevice);
  }
  ...
  // 按行复制结果回主机
  for (int y = 0; y < dstHeight; y++) {
    cudaMemcpy(dstData.data() + y * dstWidth, (char *)d_dst + y * nDstStep, dstWidth * sizeof(Npp32f),
               cudaMemcpyDeviceToHost);
  }
...
TEST_F(NPPIFilterBoxBorderTest, DISABLED_ErrorHandling) {
  ...
  status = nppiFilterBoxBorder_8u_C1R(nullptr, 32, invalidSrcRoi, oSrcOffset, nullptr, 32, oDstSizeROI, oMaskSize,
                                      oAnchor, NPP_BORDER_REPLICATE);
  EXPECT_NE(status, NPP_SUCCESS);

  // 测试无效目标尺寸
  NppiSize invalidDstRoi = {0, 0};
  status = nppiFilterBoxBorder_8u_C1R(nullptr, 32, oSrcSizeROI, oSrcOffset, nullptr, 32, invalidDstRoi, oMaskSize,
                                      oAnchor, NPP_BORDER_REPLICATE);
  EXPECT_NE(status, NPP_SUCCESS);

  // 测试无效步长
  status = nppiFilterBoxBorder_8u_C1R(nullptr, 0, oSrcSizeROI, oSrcOffset, nullptr, 0, oDstSizeROI, oMaskSize, oAnchor,
                                      NPP_BORDER_REPLICATE);
  EXPECT_NE(status, NPP_SUCCESS);

  // 测试无效滤波核尺寸
  NppiSize invalidMaskSize = {0, 0};
  status = nppiFilterBoxBorder_8u_C1R(nullptr, 32, oSrcSizeROI, oSrcOffset, nullptr, 32, oDstSizeROI, invalidMaskSize,
                                      oAnchor, NPP_BORDER_REPLICATE);
  EXPECT_NE(status, NPP_SUCCESS);
  ...
  // 测试无效边界类型
  status = nppiFilterBoxBorder_8u_C1R(nullptr, 32, oSrcSizeROI, oSrcOffset, nullptr, 32, oDstSizeROI, oMaskSize,
                                      oAnchor, static_cast<NppiBorderType>(-1));
  EXPECT_NE(status, NPP_SUCCESS);
}