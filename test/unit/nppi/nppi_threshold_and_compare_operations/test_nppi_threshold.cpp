...
TEST_F(ThresholdFunctionalTest, Threshold_8u_C1R_Less) {
    ...
    NppStatus status = nppiThreshold_8u_C1R(
        src.get(), src.step(),
        dst.get(), dst.step(),
        roi, threshold, NPP_CMP_LESS);
...
TEST_F(ThresholdFunctionalTest, Threshold_8u_C1IR_InPlace) {
    ...
    NppiSize roi = {width, height};
    NppStatus status = nppiThreshold_8u_C1IR(
...
TEST_F(ThresholdFunctionalTest, Threshold_32f_C1R_Float) {
    ...
    NppiSize roi = {width, height};
    NppStatus status = nppiThreshold_32f_C1R(
        src.get(), src.step(),
        dst.get(), dst.step(),
        roi, threshold, NPP_CMP_GREATER);
...
// 错误处理测试
TEST_F(ThresholdFunctionalTest, Threshold_ErrorHandling) {
    const int width = 16, height = 16;
    const Npp8u threshold = 128;
    ...
    status = nppiThreshold_8u_C1R(
        src.get(), src.step(),
        dst.get(), dst.step(),
        invalidRoi, threshold, NPP_CMP_LESS);
    ...
    status = nppiThreshold_8u_C1R(
        src.get(), src.step(),
        dst.get(), dst.step(),
        roi, threshold, NPP_CMP_EQ); // EQ not supported for threshold
...
TEST_F(ThresholdFunctionalTest, Threshold_BinaryImage) {
    ...
    for (size_t i = 0; i < resultData.size(); i++) {
        if (resultData[i] == threshold) lowCount++;
        else highCount++;
    }
