...
int main() {
    ...
    printf("Testing nppiSubC functions...\n");
    ...
    for (int i = 0; i < size; i++) {
        h_src_8u[i] = static_cast<Npp8u>(100 + (i % 150));
    }
    ...
    NppStatus status = nppiSubC_8u_C1RSfs(d_src_8u, width, 50, d_dst_8u, width, roi, 1);
    
    if (status == NPP_NO_ERROR) {
        printf("8u SubC: SUCCESS\n");
        ...
        printf("8u SubC: FAILED with status %d\n", status);
    }
    ...
    status = nppiSubC_32f_C1R(d_src_32f, width * sizeof(Npp32f), 25.5f, 
                              d_dst_32f, width * sizeof(Npp32f), roi_32f);
    
    if (status == NPP_NO_ERROR) {
        printf("32f SubC: SUCCESS\n");
        ...
        printf("32f SubC: FAILED with status %d\n", status);
    }
