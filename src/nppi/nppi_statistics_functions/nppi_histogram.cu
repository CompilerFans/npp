...
__global__ void nppiHistogramEven_8u_C1R_kernel_shared(const Npp8u* pSrc, int nSrcStep, 
                                                       int width, int height,
                                                       Npp32s* pHist, int nLevels, 
                                                       Npp32s nLowerLevel, Npp32s nUpperLevel) {
...
__global__ void nppiHistogramEven_8u_C1R_kernel_global(const Npp8u* pSrc, int nSrcStep, 
                                                       int width, int height,
                                                       Npp32s* pHist, int nLevels, 
                                                       Npp32s nLowerLevel, Npp32s nUpperLevel) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
...
NppStatus nppiHistogramEvenGetBufferSize_8u_C1R_Ctx_cuda(NppiSize oSizeROI, int nLevels, int* hpBufferSize) {
    ...
    *hpBufferSize = (int)(histogramSize + 1024); // Add some padding for alignment
...
NppStatus nppiHistogramEven_8u_C1R_Ctx_cuda(const Npp8u* pSrc, int nSrcStep, NppiSize oSizeROI,
    ...
    if (histBins <= 256 && imageSize < 1024 * 1024) {
        ...
        nppiHistogramEven_8u_C1R_kernel_shared<<<gridSize, blockSize, sharedMemSize, nppStreamCtx.hStream>>>(
            ...
            pHist, nLevels, nLowerLevel, nUpperLevel);
    } else {
