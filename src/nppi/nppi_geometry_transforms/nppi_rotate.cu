...
__global__ void rotate_8u_C1R_kernel(const Npp8u* __restrict__ pSrc, NppiSize oSrcSize, int nSrcStep, NppiRect oSrcROI,
                                     Npp8u* __restrict__ pDst, int nDstStep, NppiRect oDstROI,
                                     double nAngle, double nShiftX, double nShiftY) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    
    if (x >= oDstROI.width || y >= oDstROI.height) return;
    ...
    if (srcX >= oSrcROI.x && srcX < oSrcROI.x + oSrcROI.width &&
        srcY >= oSrcROI.y && srcY < oSrcROI.y + oSrcROI.height) {
 ...
 */
__global__ void rotate_32f_C1R_kernel(const Npp32f* __restrict__ pSrc, NppiSize oSrcSize, int nSrcStep, NppiRect oSrcROI,
                                      Npp32f* __restrict__ pDst, int nDstStep, NppiRect oDstROI,
                                      double nAngle, double nShiftX, double nShiftY) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
NppStatus nppiRotate_8u_C1R_Ctx_cuda(const Npp8u* pSrc, NppiSize oSrcSize, int nSrcStep, NppiRect oSrcROI,
    ...
    dim3 gridSize(
        (oDstROI.width + blockSize.x - 1) / blockSize.x,
        (oDstROI.height + blockSize.y - 1) / blockSize.y
    );
    
    // Launch kernel with the specified CUDA stream
    rotate_8u_C1R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, oSrcSize, nSrcStep, oSrcROI,
        pDst, nDstStep, oDstROI,
        nAngle, nShiftX, nShiftY
    );
...
NppStatus nppiRotate_8u_C3R_Ctx_cuda(const Npp8u* pSrc, NppiSize oSrcSize, int nSrcStep, NppiRect oSrcROI,
    ...
    // Launch kernel with the specified CUDA stream
    rotate_8u_C3R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, oSrcSize, nSrcStep, oSrcROI,
        pDst, nDstStep, oDstROI,
        nAngle, nShiftX, nShiftY
    );
...
NppStatus nppiRotate_32f_C1R_Ctx_cuda(const Npp32f* pSrc, NppiSize oSrcSize, int nSrcStep, NppiRect oSrcROI,
    ...
    // Launch kernel with the specified CUDA stream
    rotate_32f_C1R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, oSrcSize, nSrcStep, oSrcROI,
        pDst, nDstStep, oDstROI,
        nAngle, nShiftX, nShiftY
    );
