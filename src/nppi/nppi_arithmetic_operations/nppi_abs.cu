...
__device__ __forceinline__ Npp16s abs_op(Npp16s val) {
    return (val < 0) ? -val : val;
}

__device__ __forceinline__ half abs_op(half val) {
    return __habs(val);
}

__device__ __forceinline__ Npp32f abs_op(Npp32f val) {
    return fabsf(val);
}
...
__global__ void abs_kernel(const T* __restrict__ pSrc, int nSrcStep, 
...
template<typename T, int nChannels>
__global__ void abs_inplace_kernel(T* __restrict__ pSrcDst, int nSrcDstStep,
                                  int width, int height)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
template<typename T>
__global__ void abs_ac4_kernel(const T* __restrict__ pSrc, int nSrcStep, 
                              T* __restrict__ pDst, int nDstStep,
                              int width, int height)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename T>
__global__ void abs_ac4_inplace_kernel(T* __restrict__ pSrcDst, int nSrcDstStep,
                                      int width, int height)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
                                ...
                                NppiSize oSizeROI, NppStreamContext nppStreamCtx)
{
    ...
    abs_kernel<T, nChannels><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
                                       ...
                                       NppiSize oSizeROI, NppStreamContext nppStreamCtx)
{
    ...
    abs_inplace_kernel<T, nChannels><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
                                   ...
                                   NppiSize oSizeROI, NppStreamContext nppStreamCtx)
{
    ...
    abs_ac4_kernel<T><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height
    );
                                          ...
                                          NppiSize oSizeROI, NppStreamContext nppStreamCtx)
{
    ...
    abs_ac4_inplace_kernel<T><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrcDst, nSrcDstStep, oSizeROI.width, oSizeROI.height
    );
