...
template <typename T> class AlphaPremulPixelOp {
public:
  __device__ __host__ T operator()(T src, T alpha, int = 0) const {
    // NVIDIA NPP AlphaPremul (pixel alpha) behavior differs by type
    if constexpr (std::is_same_v<T, Npp8u>) {
      // 8u uses right shift for optimization
      return static_cast<T>((static_cast<int>(src) * static_cast<int>(alpha)) >> 8);
    } else if constexpr (std::is_same_v<T, Npp16u>) {
      // 16u uses division to match NVIDIA behavior
      return static_cast<T>((static_cast<int64_t>(src) * static_cast<int64_t>(alpha)) / 65535);
    } else {
      return static_cast<T>(src * alpha);
    }
...
__global__ void alphaPremulConstKernel(const T *pSrc, int nSrcStep, T alpha, T *pDst, int nDstStep, int width,
                                       int height) {
...
template <typename T, int Channels>
__global__ void alphaPremulConstInPlaceKernel(T alpha, T *pSrcDst, int nSrcDstStep, int width, int height) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
template <typename T>
__global__ void alphaPremulAC4Kernel(const T *pSrc, int nSrcStep, T *pDst, int nDstStep, int width, int height) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
template <typename T> __global__ void alphaPremulAC4InPlaceKernel(T *pSrcDst, int nSrcDstStep, int width, int height) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
NppStatus nppiAlphaPremul_8u_AC4R_Ctx(const Npp8u *pSrc1, int nSrc1Step, Npp8u *pDst, int nDstStep, NppiSize oSizeROI,
  ...
  alphaPremulAC4Kernel<Npp8u><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(pSrc1, nSrc1Step, pDst, nDstStep,
...
NppStatus nppiAlphaPremul_8u_AC4IR_Ctx(Npp8u *pSrcDst, int nSrcDstStep, NppiSize oSizeROI,
  ...
  alphaPremulAC4InPlaceKernel<Npp8u>
...
NppStatus nppiAlphaPremul_16u_AC4R_Ctx(const Npp16u *pSrc1, int nSrc1Step, Npp16u *pDst, int nDstStep,
  ...
  alphaPremulAC4Kernel<Npp16u><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(pSrc1, nSrc1Step, pDst, nDstStep,
                                                                                 oSizeROI.width, oSizeROI.height);
...
NppStatus nppiAlphaPremul_16u_AC4IR_Ctx(Npp16u *pSrcDst, int nSrcDstStep, NppiSize oSizeROI,
  ...
  alphaPremulAC4InPlaceKernel<Npp16u>
      <<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(pSrcDst, nSrcDstStep, oSizeROI.width, oSizeROI.height);
