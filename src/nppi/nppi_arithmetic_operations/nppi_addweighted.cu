...
template <typename SrcType, typename DstType> class AddWeightedInPlaceOp {
private:
  DstType alpha;
...
__global__ void addWeightedInPlaceKernel(const SrcType *pSrc, int nSrcStep, DstType *pSrcDst, int nSrcDstStep,
...
NppStatus nppiAddWeighted_8u32f_C1IR_Ctx(const Npp8u *pSrc, int nSrcStep, Npp32f *pSrcDst, int nSrcDstStep,
  ...
  addWeightedInPlaceKernel<Npp8u, Npp32f, 1><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
      pSrc, nSrcStep, pSrcDst, nSrcDstStep, oSizeROI.width, oSizeROI.height, nAlpha);
