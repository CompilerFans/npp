...
__global__ void nppiCompareC_8u_C1R_kernel(const Npp8u* pSrc, int nSrcStep,
                                           const Npp8u nConstant, Npp8u* pDst, int nDstStep,
                                           int width, int height, NppCmpOp eComparisonOperation) {
...
__global__ void nppiCompareC_16s_C1R_kernel(const Npp16s* pSrc, int nSrcStep,
                                            const Npp16s nConstant, Npp8u* pDst, int nDstStep,
                                            int width, int height, NppCmpOp eComparisonOperation) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void nppiCompareC_32f_C1R_kernel(const Npp32f* pSrc, int nSrcStep,
                                            const Npp32f nConstant, Npp8u* pDst, int nDstStep,
                                            int width, int height, NppCmpOp eComparisonOperation) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
NppStatus nppiCompareC_8u_C1R_Ctx_cuda(const Npp8u* pSrc, int nSrcStep, const Npp8u nConstant,
    ...
    nppiCompareC_8u_C1R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, nSrcStep, nConstant, pDst, nDstStep, 
        oSizeROI.width, oSizeROI.height, eComparisonOperation);
