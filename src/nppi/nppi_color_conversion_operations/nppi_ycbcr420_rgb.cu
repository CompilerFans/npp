...
__global__ void ycbcr420_to_bgr_c3_kernel(const Npp8u *pSrcY, int nSrcYStep, const Npp8u *pSrcCb, int nSrcCbStep,
                                          const Npp8u *pSrcCr, int nSrcCrStep, Npp8u *pDst, int nDstStep, int width,
                                          int height) {
  ...
  const Npp8u *rowY = reinterpret_cast<const Npp8u *>(reinterpret_cast<const char *>(pSrcY) + y * nSrcYStep);
  const Npp8u *rowCb = reinterpret_cast<const Npp8u *>(reinterpret_cast<const char *>(pSrcCb) + (y >> 1) * nSrcCbStep);
  const Npp8u *rowCr = reinterpret_cast<const Npp8u *>(reinterpret_cast<const char *>(pSrcCr) + (y >> 1) * nSrcCrStep);
  Npp8u r, g, b;
  ycbcr_to_rgb_pixel(rowY[x], rowCb[x >> 1], rowCr[x >> 1], r, g, b);

  Npp8u *dstRow = reinterpret_cast<Npp8u *>(reinterpret_cast<char *>(pDst) + y * nDstStep);
  const int index = x * 3;
  dstRow[index] = b;
  dstRow[index + 1] = g;
  dstRow[index + 2] = r;
...
cudaError_t nppiYCbCr420ToRGB_8u_P3C3R_kernel(const Npp8u *pSrcY, int nSrcYStep, const Npp8u *pSrcCb, int nSrcCbStep,
  ...
  dim3 blockSize(16, 16);
  dim3 gridSize((oSizeROI.width + blockSize.x - 1) / blockSize.x, (oSizeROI.height + blockSize.y - 1) / blockSize.y);
  ycbcr420_to_rgb_c3_kernel<<<gridSize, blockSize, 0, stream>>>(
...
cudaError_t nppiYCbCr420ToBGR_8u_P3C3R_kernel(const Npp8u *pSrcY, int nSrcYStep, const Npp8u *pSrcCb, int nSrcCbStep,
  ...
  ycbcr420_to_bgr_c3_kernel<<<gridSize, blockSize, 0, stream>>>(
      pSrcY, nSrcYStep, pSrcCb, nSrcCbStep, pSrcCr, nSrcCrStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height);
  return cudaGetLastError();
}
