...
template<typename T>
__global__ void nppiCFAToRGB_bilinear_kernel(const T* pSrc, int nSrcStep, NppiSize srcSize,
                                            NppiRect oSrcROI, T* pDst, int nDstStep,
                                            NppiBayerGridPosition eBayerGrid) {
    ...
    switch (eBayerGrid) {
        case NPPI_BAYER_BGGR:
            // B G
            // G R
            if ((abs_y % 2 == 0 && abs_x % 2 == 0)) color_type = 2; // B
            else if ((abs_y % 2 == 1 && abs_x % 2 == 1)) color_type = 0; // R
            else color_type = 1; // G
            break;
        case NPPI_BAYER_GBRG:
            // G B
            // R G
            if ((abs_y % 2 == 0 && abs_x % 2 == 1)) color_type = 2; // B
            else if ((abs_y % 2 == 1 && abs_x % 2 == 0)) color_type = 0; // R
            else color_type = 1; // G
            break;
        case NPPI_BAYER_GRBG:
            // G R
            // B G
            if ((abs_y % 2 == 0 && abs_x % 2 == 1)) color_type = 0; // R
            else if ((abs_y % 2 == 1 && abs_x % 2 == 0)) color_type = 2; // B
            else color_type = 1; // G
            break;
        case NPPI_BAYER_RGGB:
            // R G
            // G B
            if ((abs_y % 2 == 0 && abs_x % 2 == 0)) color_type = 0; // R
            else if ((abs_y % 2 == 1 && abs_x % 2 == 1)) color_type = 2; // B
            else color_type = 1; // G
            break;
    }
    ...
    if (color_type == 0) { // Red pixel
        ...
        int g_count = 0;
        int g_sum = 0;
        if (abs_x > 0) { g_sum += src_row[src_idx - 1]; g_count++; }
        if (abs_x < srcSize.width - 1) { g_sum += src_row[src_idx + 1]; g_count++; }
        if (abs_y > 0) {
            const T* prev_row = (const T*)((const char*)pSrc + (abs_y - 1) * nSrcStep);
            g_sum += prev_row[src_idx]; g_count++;
        }
        if (abs_y < srcSize.height - 1) {
            const T* next_row = (const T*)((const char*)pSrc + (abs_y + 1) * nSrcStep);
            g_sum += next_row[src_idx]; g_count++;
        }
        ...
        if (abs_x > 0 && abs_y > 0) {
            const T* prev_row = (const T*)((const char*)pSrc + (abs_y - 1) * nSrcStep);
            b_sum += prev_row[src_idx - 1]; b_count++;
        }
        ...
        if (abs_x > 0 && abs_y < srcSize.height - 1) {
            const T* next_row = (const T*)((const char*)pSrc + (abs_y + 1) * nSrcStep);
            b_sum += next_row[src_idx - 1]; b_count++;
        }
        ...
        if (abs_x > 0 && abs_y > 0) {
            const T* prev_row = (const T*)((const char*)pSrc + (abs_y - 1) * nSrcStep);
            r_sum += prev_row[src_idx - 1]; r_count++;
        }
        if (abs_x < srcSize.width - 1 && abs_y > 0) {
            const T* prev_row = (const T*)((const char*)pSrc + (abs_y - 1) * nSrcStep);
            r_sum += prev_row[src_idx + 1]; r_count++;
        ...
        if (abs_x > 0 && abs_y < srcSize.height - 1) {
            const T* next_row = (const T*)((const char*)pSrc + (abs_y + 1) * nSrcStep);
            r_sum += next_row[src_idx - 1]; r_count++;
        }
        if (abs_x < srcSize.width - 1 && abs_y < srcSize.height - 1) {
            const T* next_row = (const T*)((const char*)pSrc + (abs_y + 1) * nSrcStep);
            r_sum += next_row[src_idx + 1]; r_count++;
...
NppStatus nppiCFAToRGB_8u_C1C3R_Ctx_cuda(const Npp8u* pSrc, int nSrcStep, NppiSize oSrcSize,
    ...
    dim3 gridSize((oSrcROI.width + blockSize.x - 1) / blockSize.x,
                  (oSrcROI.height + blockSize.y - 1) / blockSize.y);
    
    nppiCFAToRGB_bilinear_kernel<Npp8u><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, nSrcStep, oSrcSize, oSrcROI, pDst, nDstStep, eGrid);
...
NppStatus nppiCFAToRGB_16u_C1C3R_Ctx_cuda(const Npp16u* pSrc, int nSrcStep, NppiSize oSrcSize,
    ...
    nppiCFAToRGB_bilinear_kernel<Npp16u><<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, nSrcStep, oSrcSize, oSrcROI, pDst, nDstStep, eGrid);
