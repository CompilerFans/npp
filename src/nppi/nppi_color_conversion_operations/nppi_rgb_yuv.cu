...
__global__ void nppiRGBToYUV_8u_C3R_kernel_impl(const Npp8u* pSrc, int nSrcStep,
...
// YUV到RGB转换的内核实现
__global__ void nppiYUVToRGB_8u_C3R_kernel_impl(const Npp8u* pSrc, int nSrcStep,
                                                 Npp8u* pDst, int nDstStep,
                                                 int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
extern "C" {
    ...
    cudaError_t nppiRGBToYUV_8u_C3R_kernel(const Npp8u* pSrc, int nSrcStep,
        ...
        nppiRGBToYUV_8u_C3R_kernel_impl<<<gridSize, blockSize, 0, stream>>>(
    ...
    cudaError_t nppiYUVToRGB_8u_C3R_kernel(const Npp8u* pSrc, int nSrcStep,
        ...
        nppiYUVToRGB_8u_C3R_kernel_impl<<<gridSize, blockSize, 0, stream>>>(
            pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height);
