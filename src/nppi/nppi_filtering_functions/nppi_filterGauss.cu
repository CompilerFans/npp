...
// 简单的3x3高斯内核
__constant__ float c_gauss3x3[9] = {
...
// 简单的5x5高斯内核
__constant__ float c_gauss5x5[25] = {
...
// 修复的8位三通道高斯滤波内核
__global__ void nppiFilterGauss_8u_C3R_kernel_fixed(const Npp8u* pSrc, int nSrcStep,
                                                     Npp8u* pDst, int nDstStep,
                                                     int width, int height, int kernelSize) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
// 修复的32位浮点单通道高斯滤波内核
__global__ void nppiFilterGauss_32f_C1R_kernel_fixed(const Npp32f* pSrc, int nSrcStep,
