...
template <typename T> struct FilterBoxAccumulator { using Type = long long; };
template <> struct FilterBoxAccumulator<Npp32f> { using Type = double; };
template <> struct FilterBoxAccumulator<Npp64f> { using Type = double; };
...
cudaError_t nppiFilterBox_8u_CxR_kernel(const Npp8u *pSrc, Npp32s nSrcStep, Npp8u *pDst, Npp32s nDstStep,
  ...
  return launchFilterBoxCxR(pSrc, nSrcStep, pDst, nDstStep, oSizeROI, oMaskSize, oAnchor, nChannels, nFilteredChannels,
                            stream);
}
