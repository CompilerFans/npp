...
__global__ void nppiCopy_32f_C3R_kernel(const Npp32f* pSrc, int nSrcStep,
                                        Npp32f* pDst, int nDstStep,
                                        int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void nppiCopy_32f_C3P3R_kernel(const Npp32f* pSrc, int nSrcStep,
                                          Npp32f* const * pDst, int nDstStep,
                                          int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
NppStatus nppiCopy_32f_C3R_Ctx_cuda(const Npp32f* pSrc, int nSrcStep,
    ...
    nppiCopy_32f_C3R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height);
...
NppStatus nppiCopy_32f_C3P3R_Ctx_cuda(const Npp32f* pSrc, int nSrcStep,
    ...
    nppiCopy_32f_C3P3R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, nSrcStep, d_pDst, nDstStep, oSizeROI.width, oSizeROI.height);
