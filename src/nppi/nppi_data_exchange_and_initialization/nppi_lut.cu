...
/**
 * CUDA kernels for NPP Linear LUT Functions
 */
...
__device__ inline int linearInterpolate(int input, int level0, int level1, int value0, int value1) {
    if (input <= level0) return value0;
    if (input >= level1) return value1;
...
__device__ inline int findLUTSegment(int input, const Npp32s* pLevels, int nLevels) {
    ...
    if (input <= pLevels[0]) return 0;
    if (input >= pLevels[nLevels - 1]) return nLevels - 2;
...
__global__ void nppiLUT_Linear_8u_C1R_kernel(const Npp8u* pSrc, int nSrcStep,
    ...
    if (x < width && y < height) {
        ...
        int result = linearInterpolate(input, 
                                     pLevels[segment], pLevels[segment + 1],
                                     pValues[segment], pValues[segment + 1]);
...
__global__ void nppiLUT_Linear_8u_C3R_kernel(const Npp8u* pSrc, int nSrcStep,
    ...
    if (x < width && y < height) {
        ...
        for (int c = 0; c < 3; c++) {
            ...
            // Perform linear interpolation
            int result = linearInterpolate(input,
                                         pLevels[c][segment], pLevels[c][segment + 1],
                                         pValues[c][segment], pValues[c][segment + 1]);
...
__global__ void nppiLUT_Linear_16u_C1R_kernel(const Npp16u* pSrc, int nSrcStep,
...
NppStatus nppiLUT_Linear_8u_C3R_Ctx_cuda(const Npp8u* pSrc, int nSrcStep,
    ...
    nppiLUT_Linear_8u_C3R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height,
        d_pValuesPtr, d_pLevelsPtr, d_nLevels);
