...
template<typename T>
__device__ inline T performThreshold(T src, T threshold, NppCmpOp op) {
    switch (op) {
        case NPP_CMP_LESS:
...
__global__ void nppiThreshold_8u_C1R_kernel(const Npp8u* pSrc, int nSrcStep,
                                           Npp8u* pDst, int nDstStep,
                                           int width, int height, Npp8u nThreshold,
...
__global__ void nppiThreshold_8u_C1IR_kernel(Npp8u* pSrcDst, int nSrcDstStep,
                                            int width, int height, Npp8u nThreshold,
                                            NppCmpOp eComparisonOperation) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void nppiThreshold_32f_C1R_kernel(const Npp32f* pSrc, int nSrcStep,
                                            Npp32f* pDst, int nDstStep,
                                            int width, int height, Npp32f nThreshold,
                                            NppCmpOp eComparisonOperation) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
NppStatus nppiThreshold_8u_C1R_Ctx_cuda(const Npp8u* pSrc, int nSrcStep,
    ...
    nppiThreshold_8u_C1R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
        ...
        nThreshold, eComparisonOperation);
